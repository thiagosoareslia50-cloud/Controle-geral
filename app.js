...
async function hashSenha(salt, senha) {
  ...
  return [...new Uint8Array(key)].map(x => x.toString(16).padStart(2, "0")).join("");
}
...
async function checkLogin(login, senha) {
  ...
  if (u.v === 2) {
    ...
    // Legacy migration to v: 2 PBKDF2
    const isLegacyMatch = (await hashSenhaLegacy(u.salt, senha)) === u.senha;
    if (isLegacyMatch) {
