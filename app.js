...
async function hashSenha(salt, senha) {
  const e = new TextEncoder();
  const k = await crypto.subtle.importKey("raw", e.encode(senha), {name: "PBKDF2"}, false, ["deriveBits"]);
  const b = await crypto.subtle.deriveBits({name: "PBKDF2", salt: e.encode(salt), iterations: 600000, hash: "SHA-256"}, k, 256);
  return [...new Uint8Array(b)].map(x => x.toString(16).padStart(2, "0")).join("");
}
...
async function checkLogin(login, senha) {
  ...
  if (u.v === 2) return (await hashSenha(u.salt, senha)) === u.senha ? u : null;
