...
// ─── Recharts global — leitura lazy dentro do DashboardPage ─────────────────
// NÃO desestrutura aqui: o CDN pode ainda não ter carregado quando app.js executa.
// A função getRC() lê window.Recharts no momento da renderização (já disponível).
function getRC() {
...
function DashboardPage({
  ...
  const bg = dark ? T.appBgDark : T.appBg,
    cardBg = dark ? T.cardBgDark : T.cardBg,
    bdr = dark ? T.borderDark : T.border;
  // Lê Recharts no momento da renderização (CDN já carregado neste ponto)
  const { ResponsiveContainer, LineChart, BarChart, CartesianGrid, XAxis, YAxis, Tooltip, Line, Bar } = getRC();
...
function HistoricoPage({
  ...
  }, ["Processo", "Data", "Órgão", "Fornecedor", "Valor", "Tipo", "Usuário", "Decisão"].map(c => /*#__PURE__*/React.createElement("th", {
    key: c,
    style: {
      padding: "10px 12px",
      textAlign: "left",
...
function App() {
  ...
  // [POLL] Carga inicial + sincronização a cada 5s + ao voltar para a aba
  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal, user) => {
    ...
    // Atualiza histórico individual (upsert na mesma chave)
