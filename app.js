...
function pdfCabecalho(doc, withLine, W) {
  ...
  doc.text("ESTADO DO MARANHÃO", W / 2, y, { align: "center" });
  y += 5;
  doc.text("PREFEITURA MUNICIPAL DE GOVERNADOR EDISON LOBÃO", W / 2, y, { align: "center" });
  y += 5;
  doc.text("CONTROLADORIA DO MUNICÍPIO", W / 2, y, { align: "center" });
  y += 5;
  if (withLine) {
...
function pdfRenderCapa(ctx, d) {
  const { doc, CML, CCW, CCA, CCB, fv } = ctx;
  let { y } = ctx;
  doc.setDrawColor(0, 0, 0);
  ...
  const capaRows = [
    ...
    ["CNPJ:", fv(d.cnpj)], ["NF/Fatura:", fv(d.nf)], ["Contrato:", fv(d.contrato)],
    ["Modalidade:", fv(d.modalidade)], ["Período de ref.:", fv(d.periodo_ref)],
    ["N° Ordem de C.:", fv(d.ordem_compra || "")], ["Data da NF.:", fv(d.data_nf)],
...
function pdfRenderTabela(ctx, d) {
  const { doc, DML, DC, vw0, vw1, LH, PAD, MIN_ROW, splitS, ensureSpace, FS, fv, scale } = ctx;
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.35);
  function dRow(lbl, val, lW, vW) {
    ...
    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.35);
    doc.rect(DML, ctx.y, lW, rH, "S");
    doc.rect(DML + lW, ctx.y, vW, rH, "S");
    const TY = ctx.y + LH * 0.9;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(FS);
    doc.text(lbl, DML + 2.5, TY);
    doc.setFont("helvetica", "normal");
    if (val) vL.forEach((l, li) => doc.text(l, DML + lW + 2.5, TY + li * LH));
    ctx.y += rH;
  }
  ...
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.35);
  doc.rect(x0, ctx.y, c0, dfH, "S");
  doc.rect(x1, ctx.y, c1, dfH, "S");
  doc.rect(x2, ctx.y, c2, dfH, "S");
  doc.rect(x3, ctx.y, c3, dfH, "S");
  doc.rect(x4, ctx.y, c4, dfH, "S");
  const mid = ctx.y + dfH / 2 + LH * 0.35;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(FS);
...
function pdfRenderChecklist(ctx, checklist, sits) {
  const { doc, ckX, CK1, CK2, CK3, LH, PAD, MIN_ROW, splitS, ensureSpace, FS, scale } = ctx;
  const ckHH = Math.max(MIN_ROW, LH + PAD);
  ensureSpace(ckHH);
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.35);
  doc.rect(ckX, ctx.y, CK1, ckHH, "S");
  doc.rect(ckX + CK1, ctx.y, CK2, ckHH, "S");
  doc.rect(ckX + CK1 + CK2, ctx.y, CK3, ckHH, "S");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(FS);
  doc.text("Item", ckX + CK1 / 2, ctx.y + ckHH / 2 + LH * 0.35, { align: "center" });
  doc.text("Descrição: Documentos – Ato", ckX + CK1 + CK2 / 2, ctx.y + ckHH / 2 + LH * 0.35, { align: "center" });
  doc.setFontSize(FS * 0.85);
  doc.text("Situação", ckX + CK1 + CK2 + CK3 / 2, ctx.y + ckHH / 2 + LH * 0.35, { align: "center" });
  doc.setFontSize(FS);
  ctx.y += ckHH;
  for (let i = 0; i < checklist.length; i++) {
    ...
    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.35);
    doc.rect(ckX, ctx.y, CK1, rH, "S");
    doc.rect(ckX + CK1, ctx.y, CK2, rH, "S");
    doc.rect(ckX + CK1 + CK2, ctx.y, CK3, rH, "S");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(FS);
    doc.setTextColor(0, 0, 0);
    const ckTY = ctx.y + LH * 0.9;
    doc.text(String(i + 1), ckX + CK1 / 2, ckTY, { align: "center" });
    dL.forEach((l, li) => doc.text(l, ckX + CK1 + 2.5, ckTY + li * LH));
    const sx = ckX + CK1 + CK2 + CK3 / 2, sy = ctx.y + rH / 2;
    if (sits[i]) pdfTick(doc, sx, sy); else pdfCross(doc, sx, sy);
    ctx.y += rH;
  }
  ctx.y += 8 * scale;
}

function pdfRenderAssinatura(ctx, d, deferir) {
  const { doc, LH, scale, SAFE, TOP_GAP, W, fv } = ctx;
  const sigH_min = LH * 5 + 12 * scale;
  const pre_gap = 3 * scale;
  if (ctx.y + pre_gap + sigH_min + 3 * scale > SAFE) {
    doc.addPage();
    ctx.y = pdfCabecalho(doc, true, W) + TOP_GAP;
  }
  ctx.y += pre_gap;
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(ctx.FS);
  doc.setTextColor(0, 0, 0);
  doc.text("Governador Edison Lobão/MA, " + fv(d.data_ateste || dtExt(new Date())), W - 19, ctx.y, { align: "right" });
  ctx.y += LH;
  doc.text("Nestes Termos:", 90, ctx.y);
  ctx.y += LH;
  doc.text(decTxt, 90, ctx.y);
  ctx.y += 12 * scale;
  doc.text(ctrlNome, W / 2, ctx.y, { align: "center" });
  ctx.y += LH;
  doc.text(ctrlCargo, W / 2, ctx.y, { align: "center" });
  ctx.y += LH;
  if (ctrlPortaria) doc.text(ctrlPortaria, W / 2, ctx.y, { align: "center" });
}
// ─── gerarPDF ─────────────────────────────────────────────────────────────────
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    const W = 210, H = 297, SAFE = H - 13;

    function ensureSpaceFn(ctx, needed) {
      if (ctx.y + needed > SAFE) {
        ctx.doc.addPage();
        ctx.y = pdfCabecalho(ctx.doc, true, W) + 8;
        ctx.doc.setFont("helvetica", "normal");
        ctx.doc.setFontSize(ctx.FS);
        ctx.doc.setTextColor(0, 0, 0);
      }
    }

    const CAB2_H = 52, TOP_GAP = 8, FOOTER_MARGIN = 13;
    const AVAIL = H - CAB2_H - TOP_GAP - FOOTER_MARGIN;

    const FS0 = 12, LH0 = 5.5, MIN_ROW0 = 7.5, PAD0 = 3.0;
    const pW = W - 30 - 19;
    const DML = 28.0;
    const DC = [24.9, 22.6, 24.4, 32.5, 33.1, 34.4];
    const CK1 = 12.7, CK2 = 139.8, CK3 = 19.4;
    const ckX = DML;

    const doc0 = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
    doc0.setFont("helvetica", "normal");
    doc0.setFontSize(FS0);
    function nL(text, w) { return doc0.splitTextToSize(fv(text), w).length; }
    function rH0(text, w) { return Math.max(MIN_ROW0, nL(text, w) * LH0 + PAD0); }
    ...
    let ctx = { doc, y: 0, FS, LH, MIN_ROW, PAD, splitS, fv, scale, SAFE, TOP_GAP, W, DML, DC, vw0, vw1, ckX, CK1, CK2, CK3, CML: 22.4, CCW: 165.1, CCA: 47.6, CCB: 117.5 };
    ctx.ensureSpace = (needed) => ensureSpaceFn(ctx, needed);

    // PÁGINA 1 — CAPA
    ctx.y = pdfCabecalho(doc, false, W) + 14;
    pdfRenderCapa(ctx, d);
    ...
    ctx.ensureSpace(pL.length * 5.8 * scale + 7);
    doc.text(pL, 30, ctx.y, { align: "justify", maxWidth: pW });
    ctx.y += pL.length * 5.8 * scale + 7;

    doc.setFont("helvetica", "normal");
    doc.setFontSize(FS);
    ctx.ensureSpace(LH * 2 + 10);
    doc.text("Ao", 30, ctx.y);
    ctx.y += LH;
    const orgL = splitS("Órgão / Departamento: " + fv(d.orgao), pW);
    doc.text(orgL, 30, ctx.y);
    ctx.y += orgL.length * LH + 5 * scale;
    ctx.ensureSpace(LH + 7);
    doc.text("Ref. Processo de Pagamento de Despesa.", 30, ctx.y);
    ctx.y += LH + 1.5 * scale;

    pdfRenderTabela(ctx, d);
    ...
    ctx.ensureSpace(aposL.length * LH + 6 * scale);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(FS);
    doc.setTextColor(0, 0, 0);
    doc.text(aposL, 30, ctx.y);
    ctx.y += aposL.length * LH + 6 * scale;

    pdfRenderChecklist(ctx, checklist, sits);

    ctx.ensureSpace(LH * 2 + 4 * scale);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(FS);
    doc.setTextColor(0, 0, 0);
    doc.text("OBSERVAÇÃO:", 30, ctx.y);
    ctx.y += 6 * scale;
    if (d.obs && d.obs.trim()) {
    ...
    pdfRenderAssinatura(ctx, d, deferir);
    ...
    return { error: e.message || "Erro ao gerar PDF." };
  }
}
