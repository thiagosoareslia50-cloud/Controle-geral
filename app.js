...
async function _sbFetch(method, chave, valor) {
  ...
  try {
    ...
    if (method === "LIST") {
      ...
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/cgel_store?chave=ilike.${encodeURIComponent(chave)}*&select=chave,valor&order=atualizado_em.asc&limit=10000`,
        { headers: _sbHeaders() }
      );
      if (!res.ok) return null;
      _sbLive = true;
...
function exportarExcel(processos, historico) {
  ...
  const COLS_ORDER = ["OBJETO", "ORGÃO", "MODALIDADE", "CONTRATO", "FORNECEDOR", "NOME FANTASIA", "CNPJ", "DOCUMENTO FISCAL", "Nº", "TIPO", "VALOR", "NÚMERO DO DOCUMENTO", "DATA", "SECRETARIO", "N° ORDEM DE COMPRA", "DATA NF", "PERÍODO DE REFERÊNCIA", "_tipoKey", "_decisao", "_registradoEm"];
  ...
  // Aba histórico — colunas ordenadas para facilitar leitura
  const HIST_COLS = [
    "Processo", "Data", "Órgão", "Fornecedor", "CNPJ", "Valor", "Tipo",
    "Decisão", "MODALIDADE", "CONTRATO", "OBJETO", "DOCUMENTO FISCAL",
    "Nº", "TIPO", "SECRETARIO", "PERÍODO DE REFERÊNCIA", "N° ORDEM DE COMPRA",
    "DATA NF", "NÚMERO DO DOCUMENTO", "_obs", "_sits", "_tipoKey", "_decisao",
    "_usuario", "_registradoEm"
  ];
  const histRows = historico.map(h => {
    const r = {};
    HIST_COLS.forEach(c => r[c] = h[c] !== undefined ? h[c] : "");
    return r;
  });
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(histRows, { header: HIST_COLS }), "Histórico");
...
function Sidebar({
  ...
  // [B2] Usa sbOnline prop (estado React do App) em vez de _sbLive (módulo — não causa re-render)
  ...
  }, isOnline ? "\u2601\uFE0F" : "\uD83D\uDD34", /*#__PURE__*/React.createElement("span", { style: { fontSize: 8, fontWeight: 700, color: isOnline ? "#4ade80" : "#f87171", letterSpacing: ".03em" } }, isOnline ? "Online" : "Offline"))));
}
...
function NovoProcessoPage({
  ...
  const handleSalvar = async () => {
    ...
    if (checarDuplicata(form.numDoc) && !editMode) {
      ...
      if (!window.confirm(
        ...
        `Clique OK para usar automaticamente o Nº ${nextProcessoNumber} (próximo disponível) e salvar.\n` +
        `Clique Cancelar para corrigir manualmente.`
      )) return;
      // Atribui automaticamente o próximo número livre e continua o salvamento imediatamente
      const numCorrigido = String(nextProcessoNumber);
      setForm(f => ({ ...f, numDoc: numCorrigido }));
      toast(`🔢 Número corrigido para ${nextProcessoNumber} — salvando...`, "info");
      // Salva diretamente com o número corrigido (sem aguardar re-render)
      setLoading(true);
      try {
        const rowCorrigido = {
          "NÚMERO DO DOCUMENTO": numCorrigido,
        ...
        await onSave(rowCorrigido, form, user);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onPaste: e => {
      // [M3] Formata automaticamente ao colar valor
      e.preventDefault();
      const pasted = (e.clipboardData || window.clipboardData).getData("text");
      upd("valor")(formatValor(pasted));
    },
...
function HistoricoPage({
  ...
  const [sortBy, setSortBy] = useState("numero"); // "numero" | "registrado"
  ...
  const filtered = useMemo(() => {
    ...
    return [...r].sort((a, b) => {
      if (sortBy === "registrado") {
        const ra = a["_registradoEm"] || "";
        const rb = b["_registradoEm"] || "";
        return ra < rb ? -sortDir : ra > rb ? sortDir : 0;
      }
      // padrão: ordenar por número do processo
      const na = parseInt(String(a["Processo"] || "0"), 10);
      const nb = parseInt(String(b["Processo"] || "0"), 10);
      return (na - nb) * sortDir;
    });
  }, [historico, q, filtDec, sortBy, sortDir]);
  ...
  // Limita exibição a 50 — os mais recentes (já ordenados por número desc)
  const exibidos = useMemo(() => filtered.slice(0, 50), [filtered]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    value: q,
    onChange: e => setQ(e.target.value),
    placeholder: "\uD83D\uDD0E  Processo, fornecedor, CNPJ, valor...",
    style: {
      ...IS(dark),
      flex: 1,
      fontSize: 13,
  ...
  }, [["Processo","numero"], ["Data",null], ["Órgão",null], ["Fornecedor",null], ["Valor",null], ["Tipo",null], ["Usuário",null], ["Registrado em","registrado"], ["Decisão",null]].map(([c, sortKey]) => /*#__PURE__*/React.createElement("th", {
    key: c,
    ...
    style: {
      ...
      textAlign: "left",
      fontWeight: 700,
      color: sortKey && sortBy === sortKey ? (dark ? MUN.gold : MUN.green) : "#475569",
      fontSize: 11,
      textTransform: "uppercase",
      letterSpacing: ".06em",
      whiteSpace: "nowrap",
      cursor: sortKey ? "pointer" : "default",
      userSelect: sortKey ? "none" : "auto",
      background: sortKey && sortBy === sortKey ? (dark ? "rgba(239,209,3,.08)" : "rgba(0,96,0,.06)") : "transparent"
    }
  }, c, sortKey && sortBy === sortKey ? (sortDir === 1 ? " ↑" : " ↓") : sortKey ? " ⇅" : "")), /*#__PURE__*/React.createElement("th", {
    style: {
      padding: "10px 12px",
      textAlign: "center",
      fontSize: 11,
...
function ConfigPage({
  ...
  // [B2] usa sbOnline (estado React) em vez de _sbLive (módulo — não re-renderiza)
  const isOnline = sbOnline ?? _sbLive;
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      margin: "12px 24px 0",
      padding: "12px 16px",
      background: isOnline ? "#052e16" : _sbReady ? "#1c1400" : "#431407",
      borderRadius: 10,
      border: `1.5px solid ${isOnline ? "#16a34a" : _sbReady ? "#ca8a04" : "#ea580c"}`,
      display: "flex",
      alignItems: "center",
      gap: 10,
      fontSize: 13,
  ...
  /*#__PURE__*/React.createElement("span", {style:{fontSize:18}}, isOnline ? "\u2705" : _sbReady ? "\u26A0\uFE0F" : "\u274C"),
  /*#__PURE__*/React.createElement("div", null,
    /*#__PURE__*/React.createElement("div", {style:{fontWeight:700, color: isOnline ? "#86efac" : _sbReady ? "#fde047" : "#fed7aa"}},
      isOnline
    ...
    /*#__PURE__*/React.createElement("div", {style:{fontSize:11, color: isOnline ? "#4ade80" : _sbReady ? "#fbbf24" : "#fb923c", marginTop:2}},
...
function App() {
  ...
  // [B2] Estado React para refletir _sbLive — módulo não causa re-render automático
  ...
  useEffect(() => {
    const refresh = async () => {
      ...
      // [B2] Sincroniza estado React com _sbLive após cada poll
      setSbOnline(_sbLive);
    ...
    // [M1] Reconecta imediatamente quando o browser volta à rede
    const onOnline = () => {
      _sbTestConnection().then(ok => {
        setSbOnline(ok);
        refresh();
      });
    };
  ...
  const handleSync = useCallback(async () => {
    ...
    // [M8] Atualiza indicador de conexão imediatamente após sync bem-sucedido
    setSbOnline(_sbLive);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onEditar: h => {
      ...
      if (proc) {
        ...
        // [B5] Fallback: processo existe apenas no histórico (não tem proc_ atômico)
        // Converte campos do histórico para o formato esperado pelo formulário de edição
        const procFromHist = {
          "NÚMERO DO DOCUMENTO": h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "",
          "DATA": h["Data"] || "",
          "ORGÃO": h["Órgão"] || h["ORGÃO"] || "",
          ...
          "CONTRATO": h["CONTRATO"] || "",
          "SECRETARIO": h["SECRETARIO"] || "",
          "DOCUMENTO FISCAL": h["DOCUMENTO FISCAL"] || "",
          "Nº": h["Nº"] || "",
          "TIPO": h["TIPO"] || "",
          "DATA NF": h["DATA NF"] || "",
          "PERÍODO DE REFERÊNCIA": h["PERÍODO DE REFERÊNCIA"] || "",
          "N° ORDEM DE COMPRA": h["N° ORDEM DE COMPRA"] || "",
          "_tipoKey": h["TipoKey"] || h["_tipoKey"] || "padrao",
          "_decisao": h["_decisao"] || (h["Decisão"] === "DEFERIDO" ? "deferir" : h["Decisão"] === "INDEFERIDO" ? "indeferir" : "deferir"),
          "_obs": h["_obs"] || "",
          "_sits": h["_sits"] || [],
          "_usuario": h["_usuario"] || ""
        };
        toast("⚠️ Processo encontrado apenas no Histórico — editando a partir dos dados disponíveis.", "warn");
        handleEditar(procFromHist);
      }
    },
