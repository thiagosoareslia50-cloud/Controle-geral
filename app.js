...
// ─── MapData ──────────────────────────────────────────────────────────────────
// ⚡ Bolt: Single-pass iteration to build MapData
// We avoid calling loops 33 times on `processos` array by building all data structures in one pass.
function buildMapData(processos) {
  ...
  const fornObjetosListMap = {}, fornContratosListMap = {}, fornModalidadesListMap = {};
  const modalContratosListMap = {}, orgaoContratosListMap = {}, orgaoModalidadesListMap = {};

  const allSecretariosSet = new Set(), allCnpjsSet = new Set(), allContratosSet = new Set();
  const allObjsHistSet = new Set(), allDocFiscaisSet = new Set(), allTiposNfSet = new Set();
  const allModalidadesSet = new Set(), allOrgaosSet = new Set(), allFornecedoresSet = new Set();
  ...
  for (const p of processos) {
    ...
    if (fornecedor) {
      ...
      if (contrato) {
        fornContrato[fornecedor] = contrato;
    ...
    if (cnpj) {
      allCnpjsSet.add(cnpj);
      if (fornecedor) cnpjForn[cnpj] = fornecedor;
    }

    if (modalidade) {
      allModalidadesSet.add(modalidade);
      if (contrato) {
        modalContrato[modalidade] = contrato;
  ...
  return {
    ...
    fornPeriodo,
    fornOrdemCompra,
    fornObjetosList: multiProcess(fornObjetosListMap),
    fornContratosList: multiProcess(fornContratosListMap),
    fornModalidadesList: multiProcess(fornModalidadesListMap),
    cnpjForn,
    modalContrato,
    modalContratosList: multiProcess(modalContratosListMap),
    objModalidade,
    objContrato,
    ...
    orgaoContratosList: multiProcess(orgaoContratosListMap),
    orgaoModalidadesList: multiProcess(orgaoModalidadesListMap)
  };
}
