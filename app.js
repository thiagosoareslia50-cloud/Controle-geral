...
async function checkLogin(login, senha) {
  ...
  // Tenta PBKDF2 (novo padrão)
  const hashAtual = await hashSenha(u.salt, senha);
  if (hashAtual === u.senha) return u;

  // Tenta Legacy SHA-256 (compatibilidade com versões anteriores)
  const hashAntigo = await hashSenhaLegacy(u.salt, senha);
  if (hashAntigo === u.senha) {
    // Atualização oportunista da senha para o novo hash PBKDF2
    u.senha = hashAtual;
    await ST.set("users", us); // Salva o banco de dados com a senha convertida
    return u;
  }
