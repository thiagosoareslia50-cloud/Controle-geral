...
async function _alocarNumeroAtomico(processosAtuais) {
  ...
  for (let t = 0; t < MAX_TENT; t++) {
    ...
    if (seq >= cand) {
      cand = seq + 1;
      while (usados.has(cand) && cand < 99999) cand++;
      const padCandNew = String(cand).padStart(4, "0");
      jaExiste = await ST.get(`proc_${padCandNew}`);
    }

    const padCandFinal = String(cand).padStart(4, "0");
    ...
    await ST.set(`proc_${padCandFinal}`, { _reservado: true, _sessao: _SESSAO_ID, _ts: Date.now() });
    await new Promise(r => setTimeout(r, 80 + t * 40));
    const leitura = await ST.get(`proc_${padCandFinal}`);
...
async function exportarExcel(processos, historico) {
  const XLSX = await loadXlsxLib();
...
async function loadXlsxLib() {
  ...
  return new Promise((res, rej) => {
    ...
    s.onload = () => window.XLSX ? (_xlsxLib = window.XLSX, res(_xlsxLib)) : rej(new Error("SheetJS não carregou"));
    s.onerror = () => rej(new Error("Falha ao carregar SheetJS"));
    document.head.appendChild(s);
  });
}
window.loadXlsxLib = loadXlsxLib;
...
function ModalConfirmacaoSenha({ org, onConfirm, onCancel, dark }) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: { fontSize: 15, fontWeight: 700, color: dark ? "#e2e8f0" : "#0f172a", marginBottom: 6 }
  }, "⚠️ Confirmar Exclusão"),
  /*#__PURE__*/React.createElement("p", {
    style: { fontSize: 12.5, color: dark ? "#cbd5e1" : "#475569", marginBottom: 12 }
  }, `Deseja realmente excluir permanentemente o órgão "${org}"?`),
  /*#__PURE__*/React.createElement("p", {
    style: { fontSize: 11.5, color: "#ef4444", marginBottom: 16 }
  }, "Esta ação é irreversível e exige a sua senha de administrador."),
  /*#__PURE__*/React.createElement("input", {
    type: "password",
    value: password,
  ...
  /*#__PURE__*/React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 4 } },
    ...
    /*#__PURE__*/React.createElement("button", {
      ...
      style: { ...BS("danger", !password.trim() || loading, dark), flex: 1, justifyContent: "center", height: 38 }
...
function NovoProcessoPage({
  ...
  const orgAtivos = useMemo(() => {
    const s = new Set(mp.allOrgaos);
    Object.keys(orgaosConfig || {}).forEach(k => s.add(k));
    return [...s].filter(o => orgaosConfig[o]?.ativo !== false && !orgaosConfig[o]?.excluido).sort();
  ...
  const activeForns = useMemo(() => {
    const allFornecedores = mp.allFornecedores;
    const suppliersWithActiveContracts = new Set();
    const suppliersWithContracts = new Set();
    Object.keys(contratosMeta || {}).forEach(k => {
      const meta = contratosMeta[k];
      if (meta.fornecedor) {
        const fUpper = meta.fornecedor.trim().toUpperCase();
        suppliersWithContracts.add(fUpper);
        if (!meta.inativo && !meta.excluido) {
          suppliersWithActiveContracts.add(fUpper);
        }
      }
    });
    return allFornecedores.filter(f => {
      const fUpper = f.trim().toUpperCase();
      if (suppliersWithContracts.has(fUpper)) {
        return suppliersWithActiveContracts.has(fUpper);
      }
      return true;
    });
  }, [mp.allFornecedores, contratosMeta]);
  const activeFornsCount = activeForns.length;
  ...
  const formatVal = n => "R$ " + n.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

  const selectedContratoProgress = useMemo(() => {
    if (!form.contrato) return null;
    const cleanC = cleanContrato(form.contrato);
    const matchKey = Object.keys(contratosMeta || {}).find(k => cleanContrato(k) === cleanC);
    const meta = matchKey ? contratosMeta[matchKey] : null;
    
    let totalExecutado = 0;
    const parseMonetarioLocal = str => {
      if (!str) return 0;
      const s = String(str).replace(/[^\d]/g, "");
      return s ? parseInt(s, 10) / 100 : 0;
    };
    const toISOLocal = dStr => {
      if (!dStr) return "";
    ...
    const cleanContratoLocal = c => {
      if (!c) return "";
      return String(c).trim().toUpperCase();
    };

    const cCleanUpper = cleanContratoLocal(form.contrato);

    (processos || []).forEach(p => {
      const rawC = p["CONTRATO"] || p["Contrato"] || p["contrato"] || p["NUMERO CONTRATO"] || p["N CONTRATO"] || p["Nº CONTRATO"] || p["CONTRATO N"] || p["NR_CONTRATO"] || p["Nr. Contrato"] || p["Número do Contrato"] || "";
      if (cleanContratoLocal(rawC) === cCleanUpper) {
        const dataIso = toISOLocal(p["DATA"] || p["Data"] || p["dataDoc"] || "");
        if (dataIso >= "2026-01-01") {
          totalExecutado += parseMonetarioLocal(p["Valor"] || p["VALOR"] || p["VALOR LÍQUIDO"] || p["Valor Líquido"] || "0");
        }
      }
    });
    ...
    (historico || []).forEach(p => {
      const rawC = p["CONTRATO"] || p["Contrato"] || p["contrato"] || p["NUMERO CONTRATO"] || p["N CONTRATO"] || p["Nº CONTRATO"] || p["CONTRATO N"] || p["NR_CONTRATO"] || p["Nr. Contrato"] || p["Número do Contrato"] || "";
      if (cleanContratoLocal(rawC) === cCleanUpper) {
        const dataIso = toISOLocal(p["DATA"] || p["Data"] || "");
        if (dataIso >= "2026-01-01") {
          totalExecutado += parseMonetarioLocal(p["Valor"] || p["VALOR"] || p["Valor Líquido"] || "0");
        }
      }
    });

    if (meta && meta.valorExecutadoManual !== undefined && meta.valorExecutadoManual !== null) {
      totalExecutado = meta.valorExecutadoManual;
    }

    const valorTotal = (meta && meta.valorTotal) || 0;
    const aditivos = (meta && meta.aditivos) || 0;
    const totalComAditivos = valorTotal + aditivos;

    const saldo = totalComAditivos > 0 ? (totalComAditivos - totalExecutado) : null;
    const progresso = totalComAditivos > 0 ? (totalExecutado / totalComAditivos) * 100 : 0;
    const validade = (meta && meta.validade) || "";
  ...
  }, [form.contrato, processos, historico, contratosMeta]);
  ...
  const onOrgChange = val => {
    ...
    const secConfig = getSecFromConfig(v);
    setForm(f => ({
      ...f,
      orgao: v,
      secretario: secConfig || mp.orgaoSecretario[v] || "",
  ...
  const onFornChange = val => {
    setIsSaved(false);
    const v = val.toUpperCase();
    let suggestedContrato = "";
    if (v && contratosMeta) {
      const match = Object.keys(contratosMeta).find(k => 
        !contratosMeta[k].inativo && 
        !contratosMeta[k].excluido && 
        contratosMeta[k].fornecedor && 
        contratosMeta[k].fornecedor.toUpperCase() === v.toUpperCase()
      );
      if (match) suggestedContrato = match;
    }
    
    if (!suggestedContrato) {
      suggestedContrato = mp.fornContrato[v] || "";
    }

    const hasDados = mp.fornCnpj[v] || mp.fornObjeto[v] || suggestedContrato;
    
    setForm(f => {
      const meta = suggestedContrato && contratosMeta ? contratosMeta[suggestedContrato] : null;
      const novCnpj = (meta && meta.cnpj) || mp.fornCnpj[v] || "";
      const novObj = (meta && meta.objeto) || "";
      const novMod = (meta && meta.modalidade) || "";
      const novOrg = (meta && meta.orgao) || "";
      ...
      return {
        ...
        objeto: novObj,
        modalidade: novMod,
        contrato: f.contrato || suggestedContrato,
        orgao: novOrg,
    ...
    if (hasDados) {
      setAutoFillMsg(v ? `Dados do histórico/contrato aplicados para "${v.slice(0,30)}"` : "");
  ...
  const onContratoChange = v => {
    setIsSaved(false);
    const cleanV = cleanContrato(v);
    const matchKey = Object.keys(contratosMeta || {}).find(k => cleanContrato(k) === cleanV);
    const meta = matchKey ? contratosMeta[matchKey] : null;
    const forn = (meta && meta.fornecedor) || "";
    const org  = (meta && meta.orgao) || "";
    const mod  = (meta && meta.modalidade) || "";
    const obj  = (meta && meta.objeto) || "";
    const metaCnpj = (meta && meta.cnpj) || "";
    
    setForm(f => {
      const novoFornecedor = forn || f.fornecedor;
      const novoOrgao = org || f.orgao;
      return {
        ...f,
        contrato: v,
        fornecedor: novoFornecedor,
        orgao:      novoOrgao,
        modalidade: mod || f.modalidade,
        objeto:     obj || f.objeto,
        cnpj:       metaCnpj || (forn ? mp.fornCnpj[forn] || "" : "") || f.cnpj,
        secretario: org ? (getSecFromConfig(org) || mp.orgaoSecretario[org] || "") : f.secretario
      };
    });
    if (forn || org || meta) {
  ...
  const cShow = useMemo(() => {
    const raw = contMode === "forn" && cFF.length ? cFF : mp.allContratos;
    return raw.filter(c => {
      const meta = contratosMeta && contratosMeta[c];
      if (meta) {
        return !meta.inativo && !meta.excluido;
      }
      return true;
    });
  }, [contMode, cFF, mp.allContratos, contratosMeta]);
  ...
  const secSug = form.orgao && !form.secretario ? getSecFromConfig(form.orgao) || mp.orgaoSecretario[form.orgao] || "" : "";
  ...
  const makeDados = () => {
    ...
    const cleanC = cleanContrato(contrato);
    const matchKey = Object.keys(contratosMeta || {}).find(k => cleanContrato(k) === cleanC);
    const meta = matchKey ? contratosMeta[matchKey] : null;
    const cnpj      = form.cnpj      || (meta ? meta.cnpj || "" : "") || (useMap ? mp.fornCnpj[forn] || "" : "");
    const modalidade= form.modalidade|| (meta ? meta.modalidade || "" : "") || (useMap ? mp.fornModalidade[forn] || mp.orgaoModalidade[org] || "" : "");
    const secretario= form.secretario|| (useMap ? getSecFromConfig(org) || mp.orgaoSecretario[org] || "" : "");
    const objeto    = form.objeto    || (meta ? meta.objeto || "" : "") || (useMap ? mp.fornObjeto[forn] || "" : "");
  ...
  const handleSalvar = async (retornarId = false) => {
    ...
    try {
      ...
      setDraftSaved(null);
      setForm(f => ({ ...f, numDoc: String(idSalvo) }));
      setIsSaved(true);
  ...
  const handleLimpar = () => {
    if (isSaved) {
      ...
      setPdfBlob(null);
      setIsSaved(false);
      toast("✨ Formulário pronto para novo cadastro!");
      return;
    }
  ...
  const ti = TINFO[form?.tipo] || TINFO["padrao"];
  const chkItems = CHK[form?.tipo] || CHK["padrao"] || [];
  const sits = getChks(form?.tipo || "padrao");
  ...
  const iStyle = { ...IS(dark), marginBottom: compact ? 6 : 14 };
  const tabSt = i => ({
    padding: "9px 16px",
    fontSize: 12.5,
    fontWeight: 600,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => { setEditMode(null); setForm(blankForm()); setChks({}); setPdfBlob(null); setIsSaved(false); },
    style: {
      marginLeft: "auto",
  ...
  /*#__PURE__*/React.createElement("span", {style:{color: draftSaved.cloud?"#16a34a":"#d97706", fontWeight:600}},
    ...
    value: typeof nextProcessoNumber === "number" ? String(nextProcessoNumber).padStart(4, "0") : (nextProcessoNumber === "..." ? "..." : String(proxNumero(processos)).padStart(4, "0")),
    gradient: T.kpi4,
    icon: "\uD83D\uDD22"
  })), /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, ["📜 Contrato", "🏢 Dados", "✅ Ateste"].map((t, i) => /*#__PURE__*/React.createElement("button", {
    key: i,
    style: tabSt(i),
    onClick: () => setTab(i)
    ...
    style: {
  ...
  }, contMode === "forn" ? "📂" : "🏢")), selectedContratoProgress && /*#__PURE__*/React.createElement("div", {
        style: {
          marginTop: 4,
          fontSize: 10.5,
          color: dark ? "#94a3b8" : "#64748b",
      ...
      }, selectedContratoProgress.saldo === null ? (
        `Saldo: Não config. · Exec: ${typeof formatVal === "function" ? formatVal(selectedContratoProgress.valorExecutado) : selectedContratoProgress.valorExecutado} · ⏳ Validade: Não cad.`
      ) : (
        `Saldo: ${typeof formatVal === "function" ? formatVal(selectedContratoProgress.saldo) : selectedContratoProgress.saldo} (${selectedContratoProgress.progresso.toFixed(1)}% us.) · ⏳ Val: ${selectedContratoProgress.validade ? selectedContratoProgress.validade.split("-").reverse().join("/") : "Não cad."}`
      ))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column"
    ...
    onChange: e => { setIsSaved(false); upd("ordemCompra")(e.target.value); },
    ...
    onChange: v => { setIsSaved(false); setForm(f => ({ ...f, tipDoc: v })); },
    dark: dark
  }), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: LS(dark)
  }, "N\xBA NF"), /*#__PURE__*/React.createElement("input", {
    value: form.numNf,
    onChange: e => { setIsSaved(false); upd("numNf")(e.target.value); },
    placeholder: "229",
    style: iStyle
  })), /*#__PURE__*/React.createElement(SearchSelect, {
    label: "Tipo NF",
    ...
    onChange: v => { setIsSaved(false); setForm(f => ({ ...f, tipNf: v })); },
    dark: dark
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
    ...
    value: form.valor,
    onChange: e => { setIsSaved(false); upd("valor")(e.target.value); },
    onBlur: e => { setIsSaved(false); upd("valor")(formatValor(e.target.value)); },
  ...
  /*#__PURE__*/React.createElement("input", {
    type: "date",
    value: form.dataNf,
    onChange: e => { setIsSaved(false); upd("dataNf")(e.target.value); },
    style: {
      ...iStyle,
      ...(isDuplicating ? {
        borderColor: "#f59e0b", background: dark ? "#2d1f00" : "#fffbeb",
  ...
  )))), tab === 1 && /*#__PURE__*/React.createElement(React.Fragment, null,
  isDuplicating && /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  /*#__PURE__*/React.createElement(SH, {
    ...
    : /*#__PURE__*/React.createElement("div", {
        ...
        value: isManualNum ? form.numDoc : "Automático " + (nextProcessoNumber ? `(Nº ${typeof nextProcessoNumber === "number" ? String(nextProcessoNumber).padStart(4, "0") : nextProcessoNumber})` : ""),
        onChange: e => { setIsSaved(false); isManualNum && upd("numDoc")(e.target.value.replace(/\D/g, "")); },
        ...
        onClick: () => {
          setIsSaved(false);
          if (!isManualNum) {
            const nextVal = typeof nextProcessoNumber === "number" ? String(nextProcessoNumber).padStart(4, "0") : String(proxNumero(processos)).padStart(4, "0");
            upd("numDoc")(nextVal);
          }
    ...
    type: "date",
    value: form.dataDoc,
    onChange: e => { setIsSaved(false); upd("dataDoc")(e.target.value); },
    ...
    value: form.periodo,
    onChange: v => { setIsSaved(false); upd("periodo")(v); },
    dark: dark,
    ...
    value: form.orgao,
    options: orgAtivos,
    onChange: v => { setIsSaved(false); onOrgChange(v); },
    dark: dark
  }), /*#__PURE__*/React.createElement("div", null, secSug && /*#__PURE__*/React.createElement("div", {
    onClick: () => { setIsSaved(false); setForm(f => ({ ...f, secretario: secSug })); },
    style: {
      fontSize: 9.5,
      color: "#16a34a",
      fontWeight: 600,
    ...
    options: secsOpts,
    onChange: v => { setIsSaved(false); setForm(f => ({ ...f, secretario: v })); },
    dark: dark
  }))), /*#__PURE__*/React.createElement(SH, {
    icon: "\uD83C\uDFE2",
    ...
    options: activeForns,
    onChange: v => { setIsSaved(false); onFornChange(v); },
    dark: dark
  }), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: LS(dark)
  }, "CNPJ / CPF *"), /*#__PURE__*/React.createElement("input", {
    value: form.cnpj,
    onChange: e => { setIsSaved(false); onCnpjChange(e.target.value); },
    ...
    value: form.nomeFan,
    onChange: e => { setIsSaved(false); upd("nomeFan")(e.target.value); },
    ...
    value: form.objeto,
    options: oShow,
    onChange: v => { setIsSaved(false); onObjChange(v); },
    dark: dark
  })), /*#__PURE__*/React.createElement("button", {
...
function DashboardPage({
  ...
  const todos = useMemo(() => {
    ...
    (processos || []).forEach(p => {
      const rawNum = String(p["NÚMERO DO DOCUMENTO"] || "").trim();
      const cleanNum = parseInt(rawNum, 10);
      const k = isNaN(cleanNum) ? rawNum : String(cleanNum);
      if (k) map.set(k, normalize(p));
    });
    (historico || []).forEach(h => {
      const rawNum = String(h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "").trim();
      const cleanNum = parseInt(rawNum, 10);
      const k = isNaN(cleanNum) ? rawNum : String(cleanNum);
      if (k) {
...
function UsuariosPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    dark: dark
  }), Object.entries(users).filter(([login]) => login !== "__schemaV").map(([login, u]) => /*#__PURE__*/React.createElement("div", {
    key: login,
    style: {
      display: "flex",
      alignItems: "center",
...
function OrgaosPage({
  ...
  const isAdmin = user?.perfil === "admin";
  const [modalExcluirOrg, setModalExcluirOrg] = useState(null);
  const mp = useMemo(() => buildMapData(processos), [processos]);

  // Merge dos órgãos dos processos com a config
  const allOrgs = useMemo(() => {
    const s = new Set([...mp.allOrgaos, ...Object.keys(orgaosConfig)]);
    return [...s].filter(org => !orgaosConfig[org]?.excluido).sort();
  }, [mp.allOrgaos, orgaosConfig]);
  ...
  // Inativação Automática Silenciosa
  React.useEffect(() => {
    const activeOrgsInHistory = new Set();
    const checkYear = dStr => {
      if (!dStr) return false;
      const match = String(dStr).match(/\d{4}/);
      return match ? parseInt(match[0], 10) >= 2025 : false;
    };
    const processItem = p => {
      const org = p["ORGÃO"] || p["ORGAO"] || "";
      if (org && checkYear(p["DATA"] || p["Data"] || p["dataDoc"] || "")) {
        activeOrgsInHistory.add(org.trim().toUpperCase());
      }
    };
    (processos || []).forEach(processItem);
    (historico || []).forEach(processItem);

    let changed = false;
    const updated = { ...orgaosConfig };
    allOrgs.forEach(org => {
      const cleanOrg = org.trim().toUpperCase();
      const cfg = updated[org] || { secretario: "", ativo: true };
      // [FIX-AUTO-INACTIVE] Apenas inativa silenciosamente se o órgão não tiver secretário definido (não configurado)
      if (!activeOrgsInHistory.has(cleanOrg) && cfg.ativo !== false && !cfg.secretario) {
        updated[org] = { ...cfg, ativo: false };
        changed = true;
      }
    });
    ...
    if (changed) {
      (async () => {
        try {
          ...
          console.warn("Silent inactation error (QuotaExceeded protection):", err);
        }
        onOrgaosChange(updated);
      })();
    }
  }, [processos, historico, allOrgs]);
  const bg = dark ? T.appBgDark : T.appBg,
    cardBg = dark ? T.cardBgDark : T.cardBg,
    bdr = dark ? T.borderDark : T.border,
    tc = dark ? T.textMainDark : T.textMain;
  ...
  const toggleAtivo = async org => {
    ...
    try {
      ...
      console.warn("Erro ao salvar orgaos_config (QuotaExceededError):", err);
    }
    onOrgaosChange(updated);
  ...
  const handleSalvarOrgao = async () => {
    ...
    const updated = { ...orgaosConfig };
    if (editingOrg) {
      const oldCfg = orgaosConfig[editingOrg] || { ativo: true };
      if (editingOrg !== novoOrg.trim()) {
        delete updated[editingOrg];
      }
      updated[novoOrg.trim()] = {
        secretario: novoSec.trim(),
        ativo: oldCfg.ativo,
        excluido: false
      };
      setEditingOrg(null);
      toast("✅ Órgão atualizado!");
    } else {
      updated[novoOrg.trim()] = {
        secretario: novoSec.trim(),
        ativo: true,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    icon: editingOrg ? "✏️" : "➕",
    title: editingOrg ? "Editar Órgão" : "Novo Órgão",
    dark: dark
  }), /*#__PURE__*/React.createElement("label", {
    style: LS(dark)
    ...
    emoji: editingOrg ? "💾" : "➕"
  }), editingOrg ? "Salvar" : "Adicionar"), editingOrg && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setEditingOrg(null);
      setNovoOrg("");
      setNovoSec("");
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      onClick: () => setFiltro(k),
      style: {
        ...BS(active ? "success" : "ghost", false, dark),
        height: 28,
        fontSize: 11,
        padding: "0 10px"
      }
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      emoji: "✏️"
    }), "Editar"), isAdmin && !ativo && /*#__PURE__*/React.createElement("button", {
      onClick: () => setModalExcluirOrg(org),
      style: {
  ...
  })))), modalExcluirOrg && /*#__PURE__*/React.createElement(ModalConfirmacaoSenha, {
    org: modalExcluirOrg,
    dark: dark,
    onCancel: () => setModalExcluirOrg(null),
    onConfirm: async password => {
      const verifiedUser = await checkLogin(user?.login, password);
      if (!verifiedUser) {
        toast("Senha incorreta.", "error");
        return false;
      }
      const updated = { ...orgaosConfig };
      updated[modalExcluirOrg] = {
        ...(updated[modalExcluirOrg] || {}),
        excluido: true,
        ativo: false
      };
      ...
      onOrgaosChange(updated);
      setModalExcluirOrg(null);
      toast("🗑️ Órgão excluído com sucesso!");
      _incrementarVersao().catch(()=>{});
      return true;
    }
  }));
...
// ─── ControladorForm ──────────────────────────────────────────────────────────
function ControladorForm({ appConfig, setAppConfig, dark, toast, compact }) {
  const ctrl = appConfig?.controlador || {};
...
function ConfigPage({
  ...
  const handleExportExcel = async () => {
    await exportarExcel(processos, historico);
  ...
  const onSave = useCallback(async (row, form, user) => {
    ...
    try {
      ...
      if (isManual) {
        ...
        const padManual = String(numManual).padStart(4, "0");
        const jaExiste = await ST.get(`proc_${padManual}`);
        if (jaExiste && !jaExiste._reservado) {
          toast(`❌ O número ${padManual} já está em uso por outro processo.`, "error");
          return null;
        }
        numAlocado = numManual;
        toast(`⚙️ Usando número manual: ${padManual}`, "info");
      } else {
      ...
      numSalvo = String(numAlocado).padStart(4, "0");
