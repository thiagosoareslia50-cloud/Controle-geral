...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    const sigH = LH * 5 + 22 * scale + 10;
    ensureSpace(doc, sigH);
    y += 6 * scale;
...
function App() {
  ...
  const handleGerarPDFBusca = useCallback(async row => {
    ...
    const d = {
      processo: row["NÚMERO DO DOCUMENTO"] || row["Processo"] || "",
      orgao: row["ORGÃO"] || row["Órgão"] || "",
      secretario: row["SECRETARIO"] || "",
      ...
      data_nf: formatData(row["DATA NF"] || row["Data"] || ""),
      data_ateste: dtExt(formatData(row["DATA"] || row["Data"] || "")),
      objeto: row["OBJETO"] || "",
