/*
================================================================================
  SISTEMA DE CONTROLE DE PROCESSOS DE PAGAMENTO — React v3.1
...
================================================================================
  ...
  [FIX-DASH] Recharts declarado globalmente — corrige crash do Dashboard
  [BUG7] PNAB – Premiação Cultural Aldir Blanc adicionado
================================================================================
*/

// ─── Recharts (global via CDN) — [FIX-DASH] ──────────────────────────────────
const {
  ResponsiveContainer,
  LineChart,
  BarChart,
...
const CHK = {
  ...
  pnab: ["Documentos pessoais (RG e CPF)", "Formulário de Inscrição e informações sobre a trajetória", "Comprovante de residência no município", "Curriculum ou Portifólio Artístico", "Publicação do resultado final", "Certidões pessoa física ou jurídica"]
};
const TINFO = {
  padrao: {
    label: "Anexo II",
...
function DashboardPage({
  ...
  const bg = dark ? T.appBgDark : T.appBg,
    cardBg = dark ? T.cardBgDark : T.cardBg,
    bdr = dark ? T.borderDark : T.border;
  // [FIX-DASH] Guard: se Recharts não estiver carregado, exibe aviso em vez de crashar
  if (!window.Recharts || !ResponsiveContainer) {
    return /*#__PURE__*/React.createElement("div", {
      style: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: bg, gap: 14, padding: 40 }
    },
      /*#__PURE__*/React.createElement("span", { style: { fontSize: 48 } }, "\u26A0\uFE0F"),
      /*#__PURE__*/React.createElement("div", { style: { fontWeight: 700, color: "#ea580c", fontSize: 16 } }, "Biblioteca de gr\xE1ficos n\xE3o carregada"),
      /*#__PURE__*/React.createElement("div", { style: { fontSize: 12, color: "#94a3b8", textAlign: "center", maxWidth: 420 } },
        "Adicione no HTML, antes do app.js:",
        /*#__PURE__*/React.createElement("pre", { style: { marginTop: 8, background: "#0f172a", color: "#7dd3fc", borderRadius: 8, padding: "10px 14px", fontSize: 11, textAlign: "left" } },
...
function App() {
  ...
  const handleImport = useCallback(rows => {
    ...
    toast("✅ " + rows.length + " registros importados. " + enriched.filter(p => p["CNPJ"]).length + " processos existentes enriquecidos.", "info");
  }, [processos, toast]);
