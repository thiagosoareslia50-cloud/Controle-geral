...
// ─── MapData ──────────────────────────────────────────────────────────────────
// ⚡ Bolt: Replace multi-pass multi/dct/lst abstractions with a single O(N) pass for performance
function buildMapData(processos) {
  ...
  const cnpjForn = {}, modalContrato = {}, modalContratosList = {};
  const objModalidade = {}, objContrato = {};
  ...
  for (let i = 0; i < processos.length; i++) {
    ...
    const ordem = p["N° ORDEM DE COMPRA"] ? String(p["N° ORDEM DE COMPRA"]).trim() : "";
    ...
    if (forn) allFornecedores.add(forn);
    ...
    if (forn) {
      if (cnpj) fornCnpj[forn] = cnpj;
      if (objeto) fornObjeto[forn] = objeto;
      if (modalidade) fornModalidade[forn] = modalidade;
      if (contrato) fornContrato[forn] = contrato;
      if (nf) fornNf[forn] = nf;
      if (docFiscal) fornTipDoc[forn] = docFiscal;
      if (tipo) fornTipNf[forn] = tipo;
      if (periodo) fornPeriodo[forn] = periodo;
      if (ordem) fornOrdemCompra[forn] = ordem;

      if (objeto) { fornObjetosList[forn] = fornObjetosList[forn] || new Set(); fornObjetosList[forn].add(objeto); }
      if (contrato) { fornContratosList[forn] = fornContratosList[forn] || new Set(); fornContratosList[forn].add(contrato); }
      if (modalidade) { fornModalidadesList[forn] = fornModalidadesList[forn] || new Set(); fornModalidadesList[forn].add(modalidade); }
    }

    if (cnpj && forn) cnpjForn[cnpj] = forn;

    if (modalidade) {
      if (contrato) {
        modalContrato[modalidade] = contrato;
        modalContratosList[modalidade] = modalContratosList[modalidade] || new Set();
    ...
    if (orgao) {
      if (contrato) { orgaoContratosList[orgao] = orgaoContratosList[orgao] || new Set(); orgaoContratosList[orgao].add(contrato); }
      if (modalidade) { orgaoModalidadesList[orgao] = orgaoModalidadesList[orgao] || new Set(); orgaoModalidadesList[orgao].add(modalidade); }
    }
  }

  // Convert sets to sorted arrays for multi mappings
  ...
  return {
    ...
    fornPeriodo,
    fornOrdemCompra,
    fornObjetosList: toSortedArray(fornObjetosList),
    fornContratosList: toSortedArray(fornContratosList),
    fornModalidadesList: toSortedArray(fornModalidadesList),
    cnpjForn,
    modalContrato,
    modalContratosList: toSortedArray(modalContratosList),
    objModalidade,
    objContrato,
    ...
    orgaoContratosList: toSortedArray(orgaoContratosList),
    orgaoModalidadesList: toSortedArray(orgaoModalidadesList)
  };
}
