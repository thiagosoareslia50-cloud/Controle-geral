...
function buildMapData(processos) {
  // ⚡ Bolt: Single-pass iteration to extract multiple maps, sets, and lists
  // Avoids O(27*N) performance hit from running 27 separate iterations over the large processos array.
  const res = {
    orgaoSecretario: {}, orgaoContrato: {}, orgaoModalidade: {}, fornCnpj: {},
    fornObjeto: {}, fornModalidade: {}, fornContrato: {}, fornNf: {},
    ...
    objModalidade: {}, objContrato: {}, orgaoContratosList: {}, orgaoModalidadesList: {}
  };
  ...
  for (let i = 0; i < processos.length; i++) {
    ...
    if (fornecedor) {
      ...
      if (tipDoc) res.fornTipDoc[fornecedor] = tipDoc;
      ...
      sets.allFornecedores.add(fornecedor);
    }
    ...
    if (modalidade) {
      ...
      sets.allModalidades.add(modalidade);
    }

    if (objeto) {
      if (modalidade) res.objModalidade[objeto] = modalidade;
      if (contrato) res.objContrato[objeto] = contrato;
      sets.allObjsHist.add(objeto);
    }

    if (secretario) sets.allSecretarios.add(secretario);
    if (cnpj) sets.allCnpjs.add(cnpj);
    if (contrato) sets.allContratos.add(contrato);
    if (tipDoc) sets.allDocFiscais.add(tipDoc);
    if (tipo) sets.allTiposNf.add(tipo);
  }

  // Convert sets to sorted arrays
  const toSortedArray = (set) => [...set].sort();

  for (const k in sets) res[k] = toSortedArray(sets[k]);

  const multiKeys = ['fornObjetosList', 'fornContratosList', 'fornModalidadesList', 'modalContratosList', 'orgaoContratosList', 'orgaoModalidadesList'];
  for (const k of multiKeys) {
    const obj = res[k];
    for (const key in obj) obj[key] = toSortedArray(obj[key]);
  }
