...
const CHK = {
  ...
  // [BUG7] PNAB – Premiação Cultural Aldir Blanc
  pnab: ["Validação do Documento Fiscal", "Emissão e Autenticação das Certidões Negativas", "Conformidade com as diretrizes da Lei Aldir Blanc (PNAB)", "Disponibilidade de Saldos Orçamentários PNAB", "Comprovação de Atividade Cultural do Proponente/Entidade", "Documentação do Proponente/Entidade (Ato Constitutivo, CPF/CNPJ)"]
};
const TINFO = {
  padrao: {
    label: "Anexo II",
...
// ── Cores municipais: Ouro #EFD103 | Azul #0040E0 (sidebar/primário) | Verde #006000 (acentos)
// NOTA: swap visual aplicado — sidebar/botões primários em azul, acentos em verde
const MUN = {
  gold: "#EFD103",
  goldDk: "#b89d00",
  goldXdk: "#7a6500",
  green: "#0040E0",   // visualmente azul (swap intencional)
  greenDk: "#002da0",
  greenXdk: "#001560",
  greenMid: "#1a2a6a",
  blue: "#006000",    // visualmente verde (swap intencional)
  blueDk: "#003d00",
  blueXdk: "#001800"
};
const T = {
...
function formatData(raw) {
  ...
  // [BUG8] Retorno antecipado: já está no formato dd/mm/yyyy correto
...
function exportarExcel(processos, historico) {
  ...
  const COLS_ORDER = ["OBJETO", "ORGÃO", "MODALIDADE", "CONTRATO", "FORNECEDOR", "NOME FANTASIA", "CNPJ", "DOCUMENTO FISCAL", "Nº", "TIPO", "VALOR", "NÚMERO DO DOCUMENTO", "DATA", "SECRETARIO", "N° ORDEM DE COMPRA", "DATA NF", "PERÍODO DE REFERÊNCIA"];
  ...
  try {
    ...
    try {
      ...
      if (r[0]) {
        ...
        // [BUG3] Mapear nomes crus das colunas SQLite para os nomes canônicos do histórico
        const HIST_MAP = {
          "processo": "Processo", "num": "Processo", "numero": "Processo",
          "data": "Data",
          "orgao": "Órgão", "org": "Órgão", "secretaria": "Órgão",
          "fornecedor": "Fornecedor", "credor": "Fornecedor", "empresa": "Fornecedor",
          "valor": "Valor",
          "tipo": "Tipo", "tiponf": "Tipo", "tipo_nf": "Tipo",
          "tipokey": "TipoKey", "tipo_key": "TipoKey",
          "decisao": "Decisão", "decisão": "Decisão", "status": "Decisão",
          "cnpj": "CNPJ", "cpf": "CNPJ",
          "modalidade": "MODALIDADE",
          "contrato": "CONTRATO",
          "objeto": "OBJETO",
          "documentofiscal": "DOCUMENTO FISCAL", "documento_fiscal": "DOCUMENTO FISCAL",
          "nf": "Nº", "numeronf": "Nº", "numero_nf": "Nº",
          "secretario": "SECRETARIO",
          "periodoreferencia": "PERÍODO DE REFERÊNCIA", "periodo": "PERÍODO DE REFERÊNCIA",
          "ordemcompra": "N° ORDEM DE COMPRA", "ordem_compra": "N° ORDEM DE COMPRA",
          "datanf": "DATA NF", "data_nf": "DATA NF",
          "numerodocumento": "NÚMERO DO DOCUMENTO"
        };
        const canonHist = c => {
          const k = String(c).trim().toLowerCase().replace(/[\s_/]/g, "").normalize("NFD").replace(/[\u0300-\u036f]/g, "");
          return HIST_MAP[k] || c;
        };
        for (const row of values) {
          const o = {};
          columns.forEach((c, i) => {
            const key = canonHist(c);
            o[key] = row[i] ?? "";
          });
          // [BUG5] Converter chave interna do tipo para label legível
          if (o["Tipo"] && TINFO[o["Tipo"]]) {
            o["Tipo"] = TINFO[o["Tipo"]].label;
          }
          historico.push(o);
        }
      }
...
const IS = dark => ({
  ...
  fontSize: 13,
  borderRadius: 9,
  border: `1.5px solid ${dark ? MUN.greenDk : "#b8c8e8"}`,
  background: dark ? "rgba(0,60,0,.35)" : "#f6f8ff",
...
const BS = (v = "primary", dis = false, dark = false) => {
  ...
  const vv = {
    ...
    ghost: {
      ...
      color: dark ? "#7a9aba" : "#3a5680",
      border: `1px solid ${dark ? MUN.greenDk : "#b0c4e0"}`
    }
  };
  return {
    ...base,
...
function SearchSelect({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      background: o === value ? dark ? "#003ab0" : "#eff6ff" : "transparent",
      borderBottom: `1px solid ${dark ? "#0f1a12" : "#f8fafc"}`
    },
    onMouseEnter: e => e.currentTarget.style.background = dark ? "#003fc0" : "#f0f9ff",
    onMouseLeave: e => e.currentTarget.style.background = o === value ? dark ? "#003ab0" : "#eff6ff" : "transparent"
  }, o)))));
}
...
function PeriodoInput({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      color: dark ? "#e2e8f0" : "#1e293b",
      fontWeight: 600,
      borderBottom: `1px solid ${dark ? "#1e2d28" : "#f1f5f9"}`
    },
    onMouseEnter: e => e.currentTarget.style.background = dark ? "#1e2d28" : "#f5f3ff",
...
function ShortcutsModal({
  ...
  const bg = dark ? "#002060" : "#fff",
    bdr = dark ? "#1e2d28" : "#e8ecf4",
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      fontWeight: 700,
      border: `1px solid ${dark ? "#2d4030" : "#cbd5e1"}`
    }
  }, k))))));
}
...
function NovoProcessoPage({
  ...
  // [BUG1] Guard contra processos undefined: usa (processos||[]) para evitar TypeError em .some()
  ...
  const makeDados = () => {
    ...
    const tipDoc = form.tipDoc || mp.fornTipDoc[forn] || "";
    const periodo = form.periodo || mp.fornPeriodo[forn] || "";
    const tipNf = form.tipNf || mp.fornTipNf[forn] || "";
    return {
  ...
  const handleSalvar = async () => {
    ...
    if (checarDuplicata(form.numDoc) && !editMode) {
      if (!window.confirm(`⚠️ Número ${form.numDoc} já existe no banco de dados.\n\nUsar o número ${proxNumero(processos)} (próximo disponível)?\n\nClique CANCELAR para manter ${form.numDoc} (criará duplicata).`)) {
        // User chose to use next available number
        const autoNum = String(proxNumero(processos));
        setForm(f => ({
          ...f,
          numDoc: autoNum
        }));
        return; // let user review the changed number
      }
    }
    setLoading(true);
    try {
      ...
      setDraftSaved(null);
      // Calcular próximo número APÓS salvar (evita stale closure de proxNumero)
      const nextNum = String(proxNumero([...processos, row]));
      setForm({
        ...blankForm(),
        numDoc: nextNum
      });
  ...
  // [BUG2] Pegar o processo com o maior número de documento, não apenas o primeiro do array
  const ultimoProcesso = processos.length
    ? processos.reduce((max, p) =>
        parseInt(String(p["NÚMERO DO DOCUMENTO"] || "0"), 10) >
        parseInt(String(max["NÚMERO DO DOCUMENTO"] || "0"), 10) ? p : max)
    : null;
  ...
  const tabSt = i => ({
    ...
    border: "none",
    background: "transparent",
    borderBottom: `2px solid ${tab === i ? MUN.blue : "transparent"}`,
    color: tab === i ? MUN.blue : "#9ca3af",
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        color: act ? ti2.cor : dark ? "#4a6454" : "#64748b",
        lineHeight: 1.3
      }
    }, ti2.label));
  })), /*#__PURE__*/React.createElement("div", {
  ...
  }, "\u26A0\uFE0F N\xFAmero j\xE1 existe \u2014 ser\xE1 criada duplicata"), !editMode && !checarDuplicata(form.numDoc) && form.numDoc && parseInt(form.numDoc, 10) !== proxNumero(processos) && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 10.5,
      color: "#f59e0b",
      marginTop: -10,
  ...
  }, "\u26A0\uFE0F Pr\xF3ximo sugerido: ", proxNumero(processos), " \u2014 alterar pode criar lacuna na numera\xE7\xE3o")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: LS(dark)
    ...
    style: {
      ...
      height: "auto",
      resize: "vertical",
      borderColor: dark ? "#3b5040" : "#fde68a",
...
function BuscarPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      borderBottom: `1px solid ${bdr}`,
      background: i % 2 === 0 ? cardBg : dark ? "#131f14" : "#fafbfc"
    },
    onMouseEnter: e => e.currentTarget.style.background = dark ? "#1e2d28" : "#eff6ff",
    onMouseLeave: e => e.currentTarget.style.background = i % 2 === 0 ? cardBg : dark ? "#131f14" : "#fafbfc"
...
function DashboardPage({
  ...
  const rechartsOk = !!(window.Recharts && ResponsiveContainer && LineChart && BarChart);
  ...
  const anos = useMemo(() => {
    ...
    processos.forEach(p => {
      ...
      const m = d.match(/(\d{4})/g);
      if (m) {
        const y = m.find(v => parseInt(v) > 2000);
        if (y) s.add(y);
      }
    });
  ...
  const porMes = useMemo(() => {
    ...
    filtered.forEach(p => {
      ...
      // Suporta: "dd/mm/yyyy", "yyyy-mm-dd", "DD de mês de YYYY"
      let ym = "";
      const dm = raw.match(/(\d{2})\/(\d{2})\/(\d{4})/);
      if (dm) ym = dm[3] + "-" + dm[2]; // yyyy-MM
      else {
        const ym2 = raw.match(/(\d{4})-(\d{2})/);
        if (ym2) ym = ym2[0];
      }
      if (ym) m[ym] = (m[ym] || 0) + 1;
    });
  ...
  if (!rechartsOk) {
    return /*#__PURE__*/React.createElement("div", {
      style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: bg, flexDirection: "column", gap: 12, padding: 40 }
    },
      /*#__PURE__*/React.createElement("span", { style: { fontSize: 48 } }, "\u26A0\uFE0F"),
      /*#__PURE__*/React.createElement("div", { style: { fontWeight: 700, color: "#ea580c", fontSize: 16 } }, "Biblioteca de gr\xE1ficos (Recharts) n\xE3o carregada"),
      /*#__PURE__*/React.createElement("div", { style: { fontSize: 12, color: "#94a3b8", textAlign: "center", maxWidth: 400 } }, "Adicione o script do Recharts no seu HTML antes de app.js e recarregue a p\xE1gina:\u00A0",
        /*#__PURE__*/React.createElement("code", { style: { fontSize: 11, background: "#1e293b", color: "#7dd3fc", padding: "2px 6px", borderRadius: 4 } }, "<script src=\"https://cdnjs.cloudflare.com/ajax/libs/recharts/2.12.7/Recharts.min.js\"></script>")
      )
    );
  }
  return /*#__PURE__*/React.createElement("div", {
...
function HistoricoPage({
  ...
  const filtered = useMemo(() => {
    ...
    // [BUG4] Ordenar decrescente por número de processo (mais recente primeiro)
    return [...r].sort((a, b) =>
      parseInt(String(b["Processo"] || "0"), 10) - parseInt(String(a["Processo"] || "0"), 10));
  }, [historico, q, filtDec]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("tr", {
      ...
      onMouseEnter: e => e.currentTarget.style.background = dark ? "#1e2d28" : "#f0f9ff",
      onMouseLeave: e => e.currentTarget.style.background = i % 2 === 0 ? cardBg : dark ? "#131f14" : "#fafbfc"
      ...
      style: {
        ...
        padding: "2px 8px",
        borderRadius: 5,
        background: isDef ? "#0d1530" : "#450a0a",
  ...
  }, filtered.length, " registro(s) \xB7 Total: ", historico.length)));
}
...
function ConfigPage({
  ...
  }, "Aparece na assinatura de todos os PDFs gerados.")))), isAdmin && /*#__PURE__*/React.createElement("div", {
    style: {
      margin: "0 24px 24px",
      background: cardBg,
      borderRadius: 14,
...
function App() {
  ...
  // [BUG9] useMemo: proxNumero percorre todo o array; só recalcular quando processos mudar
  const proxNum = useMemo(() => proxNumero(processos), [processos]);
  ...
  const salvarProcessos = async p => {
    // [BUG10] Atualizar estado só após confirmar persistência, para preservar rascunho em caso de falha
    const ok = await ST.set("processos", p);
    if (ok === false) throw new Error("Falha ao salvar processos no storage.");
    setProcessos(p);
  };
  ...
  const onSave = useCallback(async (row, form) => {
    ...
    const novosProcessos = [...processos, novoItem];
    ...
    const hRow = {
      ...
      // Campos completos para permitir PDF correto a partir do histórico
      "CNPJ": row["CNPJ"] || "",
      "MODALIDADE": row["MODALIDADE"] || "",
      ...
      "_obs": form.obs,
      // [BUG5] getChks não existe no escopo de App; row["_sits"] já vem preenchido pelo handleSalvar
      "_sits": row["_sits"] || []
    };
  ...
  const handleGerarPDFBusca = useCallback(async row => {
    ...
    const d = {
      ...
      periodo_ref: r2["PERÍODO DE REFERÊNCIA"] || mpBusca.fornPeriodo[forn2] || "",
      ...
      tipo_nf: r2["TIPO"] || mpBusca.fornTipNf[forn2] || "",
      obs: r2["_obs"] || "",
      controlador: appConfig?.controlador || {}
    };
  ...
  const handleImport = useCallback(rows => {
    ...
    // Enriquecer processos existentes que não estão na planilha mas têm mesmo fornecedor
    const fornMap = {};
    rows.forEach(r => {
    ...
    const enriched = processos.filter(p => !rowMap[String(p["NÚMERO DO DOCUMENTO"])]).map(p => {
      ...
      if (!ref) return p;
      // Preencher apenas campos vazios com dados do histórico do mesmo fornecedor
      return {
        ...p,
    ...
    const allNums = rows.map(r => parseInt(String(r["NÚMERO DO DOCUMENTO"] || ""), 10)).filter(n => !isNaN(n));
    const maxNum = allNums.length ? Math.max(...allNums) : 0;
    toast("✅ " + rows.length + " registros importados. Último Nº: " + maxNum + ". Próximo: " + (maxNum + 1) + ".", "info");
  }, [processos, toast]);
