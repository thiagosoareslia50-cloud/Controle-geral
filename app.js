...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const normCol = c => String(c).trim().toUpperCase().replace(/\xa0/g, " ").replace(/\n|\t/g, " ").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, " ").trim();
...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    function nLines(text, w) {
      return doc0.splitTextToSize(fv(text), w).length;
    }
    function rowH0(text, w) {
      return Math.max(MIN_ROW0, nLines(text, w) * LH0 + PAD0);
    }
    const lbParacer = doc0.splitTextToSize("PARECER DE VERIFICAÇÃO E ANÁLISE DOCUMENTAL Nº " + fv(d.processo) + " (LIBERAÇÃO PARA PAGAMENTO)", pW);
    ...
    const dtH = [rowH0(d.objeto, vw0),
    // OBJETO
    rowH0(d.orgao, vw1),
    // Secretaria
    rowH0(d.fornecedor, vw1),
    // Fornecedor/Credor
    rowH0(d.modalidade, vw1),
    // Modalidade
    rowH0(d.contrato, vw1),
    // Contrato
    rowH0(d.cnpj, vw1),
    // CNPJ
    ...
    function dRow(lbl, val, lW, vW) {
      ...
      if (forceOnePage === false && y + rH > SAFE) {
        doc.addPage();
        y = cabecalho(doc, true);
        y += TOP_GAP;
      }
    ...
    // Linha Documento Fiscal (5 colunas)
    {
      ...
      if (forceOnePage === false && y + dfH > SAFE) {
        doc.addPage();
        y = cabecalho(doc, true);
        y += TOP_GAP;
      }
    ...
    doc.text(aposL, 30, y);
    y += aposL.length * LH + GAP6;

    // ── Checklist ──────────────────────────────────────────────────────────────
    const ckHH = Math.max(MIN_ROW, LH + PAD);
    if (forceOnePage === false && y + ckHH > SAFE) {
      doc.addPage();
      y = cabecalho(doc, true);
      y += TOP_GAP;
    }
...
function PageHeader({
  ...
  const cBg = dark ? "#061008" : "#f8faf4",
    bdr = dark ? MUN.greenDk : "#c8d8b8";
  return /*#__PURE__*/React.createElement("div", {
    style: {
      background: cBg,
...
function SH({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      flex: 1,
      height: 1,
      background: dark ? MUN.greenDk : "#c8d8b8",
      marginLeft: 4
    }
  }));
}
function SearchSelect({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      ...
      border: `1.5px solid ${dark ? "#2d4060" : "#bfdbfe"}`,
      borderRadius: 8,
      marginTop: 2,
    ...
    style: {
      ...
      background: o === value ? dark ? "#1e2d40" : "#eff6ff" : "transparent",
      borderBottom: `1px solid ${dark ? "#0f1a2e" : "#f8fafc"}`
    },
    onMouseEnter: e => e.currentTarget.style.background = dark ? "#1e2d40" : "#f0f9ff",
    onMouseLeave: e => e.currentTarget.style.background = o === value ? dark ? "#1e2d40" : "#eff6ff" : "transparent"
  }, o)))));
}
...
function LoginPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      alignItems: "center",
      justifyContent: "center",
      background: "linear-gradient(135deg,#020b01 0%,#051a04 60%,#030820 100%)"
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      width: 380,
  ...
  const NavItem = ({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        cursor: "pointer",
        transition: "all .15s",
        background: active ? "rgba(59,110,248,.22)" : "transparent",
        border: active ? "1px solid rgba(59,110,248,.35)" : "1px solid transparent"
      }
    }, /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 15,
      ...
      style: {
        ...
        color: active ? MUN.gold : "rgba(255,255,255,.55)"
      }
    }, label));
  };
  return /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      display: "flex",
      flexDirection: "column",
      background: "linear-gradient(180deg,#061a04 0%,#02080a 60%,#030d1a 100%)",
      height: "100vh",
      position: "sticky",
      top: 0,
      borderRight: "1px solid " + MUN.greenDk,
      overflowY: "auto",
      overflowX: "hidden"
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      borderBottom: "1px solid " + MUN.greenDk,
      textAlign: "center"
    }
  }, /*#__PURE__*/React.createElement(Brasao, {
    size: 52,
    ...
    style: {
      margin: "8px 10px 0",
      padding: "8px 12px",
      background: "rgba(0,64,224,.18)",
      borderRadius: 10,
      border: "1px solid rgba(0,64,224,.35)",
      display: "flex",
      justifyContent: "space-between",
      alignItems: "center"
    }
    ...
    style: {
      ...
      borderTop: "1px solid " + MUN.greenDk,
      display: "flex",
      flexDirection: "column",
      gap: 6
    }
...
function NovoProcessoPage({
  ...
  const formFromRow = useCallback(row => ({
    numDoc: String(proxNumero(processos)),
    dataDoc: todayISO(),
    periodo: row["PERÍODO DE REFERÊNCIA"] || "",
    orgao: row["ORGÃO"] || "",
    secretario: row["SECRETARIO"] || "",
    fornecedor: row["FORNECEDOR"] || "",
    cnpj: row["CNPJ"] || "",
    nomeFan: row["NOME FANTASIA"] || "",
    modalidade: row["MODALIDADE"] || "",
    contrato: row["CONTRATO"] || "",
    ordemCompra: row["N° ORDEM DE COMPRA"] || "",
    tipDoc: row["DOCUMENTO FISCAL"] || "",
    numNf: row["Nº"] || "",
    tipNf: row["TIPO"] || "",
    valor: "",
    dataNf: todayISO(),
  ...
  useEffect(() => {
    ...
    setForm({
      numDoc: row["NÚMERO DO DOCUMENTO"] || "",
      dataDoc: row["DATA"] || todayISO(),
      periodo: row["PERÍODO DE REFERÊNCIA"] || "",
      orgao: row["ORGÃO"] || "",
      secretario: row["SECRETARIO"] || "",
      fornecedor: row["FORNECEDOR"] || "",
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, contMode === "forn" ? "📂" : "🏢"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    style: LS(dark)
  }, "Ordem de Compra"), /*#__PURE__*/React.createElement("input", {
    ...
    style: {
      ...
      background: dark ? "#100d00" : "#fffbeb"
    }
  }))), /*#__PURE__*/React.createElement("div", {
    style: {
      background: dark ? "#0d1421" : "#f8faff",
      borderRadius: 12,
      padding: "14px 16px",
      border: `1px solid ${bdr}`
    }
  ...
  }, sits.filter(Boolean).length, "/", chkItems.length, " itens verificados")))))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 10,
      flexWrap: "wrap"
    ...
    disabled: loading,
    style: {
      ...BS("orange", loading, dark),
      flex: "1 1 130px"
    }
  }, /*#__PURE__*/React.createElement(BtnIco, {
    emoji: loading ? "⏳" : "📝"
  }), loading ? "Gerando..." : "Gerar Word"), /*#__PURE__*/React.createElement("button", {
...
function BuscarPage({
  ...
  }, String(p[c] || "").slice(0, 60))), /*#__PURE__*/React.createElement("td", {
    style: {
      padding: "6px 10px"
    }
  }, /*#__PURE__*/React.createElement("div", {
...
function App() {
  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal) => {
    ...
    const updated = processos.map(p => String(p["NÚMERO DO DOCUMENTO"]) === String(numOriginal) ? novoItem : p);
