...
// ─── MapData ──────────────────────────────────────────────────────────────────
// ⚡ Bolt: buildMapData now uses a single O(N) iteration over `processos` instead of 30+ separate passes, reducing execution time significantly
function buildMapData(processos) {
  ...
  for (let i = 0; i < processos.length; i++) {
    ...
    const tipDoc = p["DOCUMENTO FISCAL"] ? String(p["DOCUMENTO FISCAL"]).trim() : "";
    const tipNf = p["TIPO"] ? String(p["TIPO"]).trim() : "";
    ...
    if (fornecedor) {
      allFornecedores.add(fornecedor);
  ...
  return {
    ...
    fornPeriodo,
    fornOrdemCompra,
    fornObjetosList: sortMapSets(fornObjetosList),
    fornContratosList: sortMapSets(fornContratosList),
    fornModalidadesList: sortMapSets(fornModalidadesList),
    cnpjForn,
    modalContrato,
    modalContratosList: sortMapSets(modalContratosList),
    objModalidade,
    objContrato,
    allSecretarios: sortSet(allSecretarios),
    allCnpjs: sortSet(allCnpjs),
    allContratos: sortSet(allContratos),
    allObjsHist: sortSet(allObjsHist),
    allDocFiscais: sortSet(allDocFiscais),
    allTiposNf: sortSet(allTiposNf),
    allModalidades: sortSet(allModalidades),
    allOrgaos: sortSet(allOrgaos),
    allFornecedores: sortSet(allFornecedores),
    orgaoContratosList: sortMapSets(orgaoContratosList),
    orgaoModalidadesList: sortMapSets(orgaoModalidadesList)
  };
}
