...
================================================================================
  ...
  [POLL] Polling a cada 5 s + visibilitychange reconstroem estado a partir de
         todos os "proc_*" e "hist_*" do Supabase → 100% sincronizado
  [LIST] _sbFetch("LIST") consulta prefixo no Supabase via PostgREST LIKE
================================================================================
*/

// ─── Recharts global — FIX-DASH (corrige crash do Dashboard) ────────────────
const {
  ResponsiveContainer, LineChart, BarChart,
  CartesianGrid, XAxis, YAxis, Tooltip, Line, Bar
} = window.Recharts || {};
...
async function _sbFetch(method, chave, valor) {
  ...
  try {
    ...
    // [LIST] Busca todas as chaves com prefixo (ex: "proc_" → todos os processos)
    if (method === "LIST") {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/cgel_store?chave=like.${encodeURIComponent(chave + "%")}&select=chave,valor&order=atualizado_em.asc`,
        { headers }
      );
      if (!res.ok) return null;
      return await res.json(); // [{chave, valor}, ...]
    }
  } catch { return null; }
}
...
const ST = {
  async get(k) {
    // 1. Tenta Supabase (banco compartilhado em nuvem)
    if (_sbReady) {
      try {
    ...
    // 2. Fallback: localStorage (dados locais)
    try {
      const raw = localStorage.getItem("cgel_" + k);
      if (raw !== null) return JSON.parse(raw);
    } catch {}
    // 3. Fallback final: memória
    return MEM[k] ?? null;
  },
  async set(k, v) {
    MEM[k] = v;
    const serialized = JSON.stringify(v);
    // 1. Salva no Supabase (compartilhado entre todos os PCs)
    if (_sbReady) {
      try { await _sbFetch("POST", k, serialized); } catch {}
    }
    // 2. Salva também no localStorage (cache local / fallback offline)
    try {
  ...
  async del(k) {
    delete MEM[k];
    if (_sbReady) {
      try { await _sbFetch("DELETE", k); } catch {}
    }
    try { localStorage.removeItem("cgel_" + k); } catch {}
    return true;
  },
  // [LIST] Retorna [{key, value}] de todas as entradas com o prefixo
  async list(prefix) {
    if (_sbReady) {
      try {
        ...
        if (rows && rows.length) {
          ...
          return rows.map(r => ({ key: r.chave, value: JSON.parse(r.valor) }));
        }
      } catch {}
    }
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k && k.startsWith("cgel_" + prefix)) {
          ...
          if (raw) results.push({ key: k.slice(5), value: JSON.parse(raw) });
        }
      }
  ...
  // [DEL_PREFIX] Remove todas as chaves com prefixo (usado ao limpar banco)
  async del_prefix(prefix) {
    // Remove do Supabase via DELETE individual (sem stored procedure)
...
// Lê todos os processos individualmente ("proc_NUM") — prioridade sobre o blob.
// Fallback: blob "processos" (importações em lote, versão antiga).
// Lê TODOS os processos:
//  1. Chaves atômicas "proc_NUM" (gravadas por qualquer usuário após v3.2)
//  2. Blob legado "processos" (importações Excel e versões anteriores)
// Mescla deduplicamente — proc_ individual tem prioridade sobre o blob.
async function loadAllProcessos() {
  ...
  // 1. Blob legado primeiro (menor prioridade)
  ...
  // 2. Chaves atômicas sobrescrevem (maior prioridade)
  if (atomRows && atomRows.length > 0) {
    atomRows.forEach(r => {
      if (!r.value) return;
      const k = String(r.value["NÚMERO DO DOCUMENTO"] || "");
      if (k) map.set(k, r.value);
    });
  }

  // Ordena por número crescente
...
// Lê todos os históricos individualmente ("hist_NUM") — prioridade sobre o blob.
// Lê TODOS os históricos:
//  1. Chaves atômicas "hist_NUM" (gravadas por qualquer usuário após v3.2)
//  2. Blob legado "historico" (importações Excel e versões anteriores)
// Mescla deduplicamente por número de processo — hist_ individual tem prioridade.
async function loadAllHistorico() {
  ...
  // Mapa: numero_processo → registro (hist_ tem prioridade sobre blob)
  const map = new Map();

  // 1. Insere blob legado primeiro (menor prioridade)
  if (Array.isArray(blobArr)) {
    blobArr.forEach(h => {
      const k = String(h["Processo"] || h["NÚMERO DO DOCUMENTO"] || "");
      if (k) map.set(k, h);
    });
  }

  // 2. Sobrescreve / adiciona entradas atômicas (maior prioridade — dados mais frescos)
  if (atomRows && atomRows.length > 0) {
    atomRows.forEach(r => {
      if (!r.value) return;
      const k = String(r.value["Processo"] || r.value["NÚMERO DO DOCUMENTO"] || "");
      if (k) map.set(k, r.value);
    });
  }

  // Converte para array ordenado: mais recente (maior número) primeiro
  return [...map.values()].sort((a, b) => {
...
function DashboardPage({
  ...
  const bg = dark ? T.appBgDark : T.appBg,
    cardBg = dark ? T.cardBgDark : T.cardBg,
    bdr = dark ? T.borderDark : T.border;
  // FIX-DASH: sem Recharts mostra instrução clara em vez de crashar
  if (!window.Recharts || !ResponsiveContainer) {
    return /*#__PURE__*/React.createElement("div", {
      style: { flex:1, display:"flex", flexDirection:"column", alignItems:"center",
               justifyContent:"center", background:bg, gap:18, padding:40, textAlign:"center" }
    },
      /*#__PURE__*/React.createElement("span", { style:{fontSize:56} }, "\uD83D\uDCCA"),
      /*#__PURE__*/React.createElement("div", { style:{fontWeight:800, fontSize:18, color:"#ea580c"} },
        "Biblioteca de gr\xE1ficos n\xE3o carregada"),
      /*#__PURE__*/React.createElement("div", { style:{fontSize:13, color:dark?"#94a3b8":"#64748b", maxWidth:500, lineHeight:1.7} },
        "Adicione esta linha no ", /*#__PURE__*/React.createElement("b",null,"index.html"),
        " antes do app.js e recarregue a p\xE1gina:"
      ),
      /*#__PURE__*/React.createElement("pre", {
        style:{background:"#0f172a", color:"#7dd3fc", borderRadius:10, padding:"14px 20px",
               fontSize:12, fontFamily:"monospace", userSelect:"all", overflowX:"auto", lineHeight:1.8}
      }, '<script src="https://cdnjs.cloudflare.com/ajax/libs/recharts/2.12.7/Recharts.min.js"><\\/script>'),
      /*#__PURE__*/React.createElement("button", {
        onClick: () => { try { navigator.clipboard.writeText('<script src="https://cdnjs.cloudflare.com/ajax/libs/recharts/2.12.7/Recharts.min.js"></script>'); } catch(_){} },
        style:{background:"#6366f1", color:"#fff", border:"none", borderRadius:8,
               padding:"10px 24px", fontSize:13, fontWeight:700, cursor:"pointer"}
      }, "\uD83D\uDCCB Copiar linha do script")
    );
  }
  return /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    label: "Hist\xF3rico",
    value: (historico || []).length.toLocaleString(),
    gradient: T.kpi4,
    icon: "\uD83D\uDD50"
...
function ConfigPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      margin: "12px 24px 0",
      padding: "12px 16px",
      background: _sbReady ? "#052e16" : "#431407",
      borderRadius: 10,
      border: `1.5px solid ${_sbReady ? "#16a34a" : "#ea580c"}`,
      display: "flex",
      alignItems: "center",
      gap: 10,
      fontSize: 13,
  ...
  /*#__PURE__*/React.createElement("span", {style:{fontSize:18}}, _sbReady ? "\u2601\uFE0F" : "\u26A0\uFE0F"),
  /*#__PURE__*/React.createElement("div", null,
    /*#__PURE__*/React.createElement("div", {style:{fontWeight:700, color: _sbReady ? "#86efac" : "#fed7aa"}},
      _sbReady ? "\u2601\uFE0F Supabase conectado \u2014 dados sincronizados entre todos os PCs" : "\u26A0\uFE0F Supabase N\xC3O configurado \u2014 dados salvos apenas neste navegador"
    ),
    /*#__PURE__*/React.createElement("div", {style:{fontSize:11, color: _sbReady ? "#4ade80" : "#fb923c", marginTop:2}},
...
function App() {
  ...
  // [POLL] Carga inicial + sincronização a cada 10s + ao voltar para a aba
  useEffect(() => {
    const refresh = async () => {
      ...
      if (p && p.length) setProcessos(p);
      if (h && h.length) setHistorico(h);
      if (o) setOrgaosConfig(o);
      if (a) setAppConfig(a);
      if (n && Number.isInteger(n) && n > 0) setImportedMaxNum(n);
    };
    refresh();
    const interval = setInterval(refresh, 5000); // 5 s — sync entre todos os usuários
  ...
  const onSave = useCallback(async (row, form) => {
    ...
    // [ATOM] Grava processo em chave individual → não conflita com outros usuários
    await ST.set(`proc_${numSalvo}`, novoItem);
  ...
  const onSaveEdit = useCallback(async (row, form, numOriginal) => {
    ...
    await ST.set(`proc_${numStr}`, novoItem);

    // Atualiza histórico individual (mesma chave = upsert)
    const histExist = (await ST.get(`hist_${numStr}`)) || {};
    await ST.set(`hist_${numStr}`, {
      ...histExist,
      "Decisão": form.decisao === "deferir" ? "DEFERIDO" : "INDEFERIDO",
      "Tipo": TINFO[form.tipo]?.label || form.tipo,
      "TipoKey": form.tipo
  ...
  const handleSync = useCallback(async () => {
    ...
    if (_sbReady) {
      toast("☁️ Dados sincronizados na nuvem (Supabase)!", "info");
    } else {
      toast("⚠️ Supabase não configurado — dados salvos localmente.", "info");
    }
