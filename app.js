...
function NovoProcessoPage({
  ...
  const selectedContratoProgress = useMemo(() => {
    ...
    const meta = contratosMeta ? contratosMeta[form.contrato] : null;
  ...
  const onOrgChange = val => {
    ...
    setForm(f => ({
      ...f,
      orgao: v,
      secretario: f.secretario || secConfig || mp.orgaoSecretario[v] || "",
  ...
  const onContratoChange = v => {
    setIsSaved(false);
    // Preenche campos em cascata ao selecionar contrato do histórico ou metadados
    const meta = (contratosMeta && contratosMeta[v]) || null;
    ...
    setForm(f => {
      ...
      return {
        ...
        secretario: (novoOrgao ? getSecFromConfig(novoOrgao) || mp.orgaoSecretario[novoOrgao] || "" : "") || f.secretario
      };
    });
    if (forn || org || meta) {
  ...
  const makeDados = () => {
    ...
    const meta = contrato && contratosMeta ? contratosMeta[contrato] : null;
