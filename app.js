...
async function hashSenha(salt, senha) {
  // [Sentinel Security] Upgrade un-iterated SHA-256 to PBKDF2 (100,000 iterations)
  const e = new TextEncoder();
...
async function checkLogin(login, senha) {
  ...
  const hNew = await hashSenha(u.salt, senha);
  if (hNew === u.senha) return u;

  // [Sentinel Security] Backward compatibility & opportunistic upgrade
  const hLegacy = await hashSenhaLegacy(u.salt, senha);
  if (hLegacy === u.senha) {
    u.senha = hNew;
