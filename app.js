...
//  4. Após importar planilha, ancora no maior número da planilha

function proxNumero(processos) {
  if (!processos || !processos.length) return 1;
  ...
  // Pular números já usados (garante sem lacuna em duplicatas)
...
function numeroDuplicado(num, processos, numOriginalEdicao) {
  ...
  return processos.some(p => {
    const pn = parseInt(String(p["NÚMERO DO DOCUMENTO"] || "").trim(), 10);
    // Em modo edição, ignorar o próprio número original
    if (numOriginalEdicao && pn === parseInt(String(numOriginalEdicao).trim(), 10)) return false;
    return pn === n;
  });
}

// Calcula o maior número usado em um array de processos/linhas
function maiorNumero(processos) {
  if (!processos || !processos.length) return 0;
  const nums = processos
    .map(p => parseInt(String(p["NÚMERO DO DOCUMENTO"] || "").trim(), 10))
    .filter(n => !isNaN(n) && n > 0);
