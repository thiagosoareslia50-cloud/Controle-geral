...
const CHK = {
  ...
  aldir_blanc: ["Documentos pessoais (RG e CPF);", "Formulário de Inscrição e informações sobre a trajetória;", "Comprovante de residência em Governador Edison Lobão;", "Curriculum ou Portifólio Artístico;", "Publicação do resultado final;", "Certidões pessoa física ou jurídica;"]
};
const TINFO = {
  padrao: {
    label: "Anexo II",
...
async function gerarPDF(d, tipo, deferir, checklist, sits) {
  try {
    ...
    // ── Aldir Blanc: layout especial (página única) ───────────────────────────
    if (tipo === "aldir_blanc") {
      const doc = new jsPDF({
      ...
      const pLines = doc.splitTextToSize(pTxt, W - 30);
      doc.text(pLines, 15, y + 5, {
        align: "justify",
        maxWidth: W - 30
      });
      ...
      doc.text("Ao", 15, y);
      y += 5;
      doc.text("Órgão / Departamento: " + fv(d.orgao), 15, y);
      y += 5;
      doc.text("Ref. Processo de Pagamento.", 15, y);
      y += 7;
      // Tabela principal
      const ML = 15,
        TW = W - 30,
      ...
      const tRow = (lbl, val, bold_val) => {
        const vLines = doc.splitTextToSize(fv(val), LB - 4);
        const rH = Math.max(9, vLines.length * 5.5 + 4);
        doc.rect(ML, y, LA, rH, "S");
        doc.rect(ML + LA, y, LB, rH, "S");
        const ty = y + rH / 2 + 2;
        doc.setFont("helvetica", "bold");
        doc.text(lbl, ML + 2.5, ty);
        doc.setFont("helvetica", bold_val ? "bold" : "normal");
        vLines.forEach((l, i) => doc.text(l, ML + LA + 2.5, y + 5 + i * 5.5));
        y += rH;
      };
      tRow("OBJETO:", d.objeto);
      tRow("Secretaria/Programa:", d.orgao);
      tRow("Nome/Credor:", d.fornecedor);
      tRow("Documento:", d.tipo_doc || d.nf || "");
      tRow("Modalidade:", d.modalidade);
      tRow("CNPJ/CPF Nº", d.cnpj);
      // Conta Bancária — linha com 4 colunas
      const cbH = 12;
      const C1 = LA,
        C2 = 40,
        C3 = 30,
      ...
      const x0 = ML,
        ...
        x3 = x2 + C3,
        x4 = x3 + C4;
      doc.rect(x0, y, C1, cbH, "S");
      doc.rect(x1, y, C2, cbH, "S");
      doc.rect(x2, y, C3, cbH, "S");
      doc.rect(x3, y, C4, cbH, "S");
      doc.setFont("helvetica", "bold");
      doc.text("Conta Bancária", x0 + 2.5, y + 7.5);
      doc.text("Banco:", x1 + 2.5, y + 4);
      doc.setFont("helvetica", "normal");
      doc.text(fv(d.banco || ""), x1 + 2.5, y + 9);
      doc.setFont("helvetica", "bold");
      doc.text("Agência:", x2 + 2.5, y + 4);
      doc.setFont("helvetica", "normal");
      doc.text(fv(d.agencia || "0"), x2 + 2.5, y + 9);
      doc.setFont("helvetica", "bold");
      doc.text("Conta:", x3 + 2.5, y + 4);
      doc.setFont("helvetica", "normal");
      doc.text(fv(d.conta_banc || ""), x3 + 2.5, y + 9);
      // Adicionar Valor na mesma linha à direita
      doc.setFont("helvetica", "bold");
      const rVal = "R$ " + fv(d.valor || "");
      doc.text(rVal, x4 - 2.5, y + 7.5, {
        align: "right"
      });
      y += cbH;
      y += 6;
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10.5);
      const apos = doc.splitTextToSize("Após análise e verificação da documentação constante no processo de pagamento acima citado, constatamos o seguinte:", W - 30);
      doc.text(apos, 15, y);
      y += apos.length * 5 + 5;
      // Checklist
      const CK1 = 15,
      ...
      doc.setDrawColor(0, 0, 0);
      doc.setLineWidth(0.35);
      doc.rect(ML, y, CK1, hH, "S");
      doc.rect(ML + CK1, y, CK2, hH, "S");
      doc.rect(ML + CK1 + CK2, y, CK3, hH, "S");
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10.5);
      doc.text("Item", ML + CK1 / 2, y + 6, {
        align: "center"
      });
      doc.text("Descrição: Documentos – Ato", ML + CK1 + CK2 / 2, y + 6, {
        align: "center"
      });
      doc.setFontSize(9);
      doc.text("Situação", ML + CK1 + CK2 + CK3 / 2, y + 6, {
        align: "center"
      });
      doc.setFontSize(10.5);
      y += hH;
      for (let i = 0; i < checklist.length; i++) {
        const dLines = doc.splitTextToSize(checklist[i], CK2 - 4);
        const rH = Math.max(8, dLines.length * 5 + 3);
        doc.rect(ML, y, CK1, rH, "S");
        doc.rect(ML + CK1, y, CK2, rH, "S");
        doc.rect(ML + CK1 + CK2, y, CK3, rH, "S");
        doc.setFont("helvetica", "normal");
        doc.text(String(i + 1), ML + CK1 / 2, y + rH / 2 + 2, {
          align: "center"
        });
        dLines.forEach((l, li) => doc.text(l, ML + CK1 + 2.5, y + 5 + li * 5));
        const sx = ML + CK1 + CK2 + CK3 / 2,
          sy = y + rH / 2;
        if (sits[i]) {
          doc.setDrawColor(0, 100, 0);
          doc.setLineWidth(0.5);
          doc.line(sx - 2.2, sy, sx - 0.5, sy + 2.2);
          doc.line(sx - 0.5, sy + 2.2, sx + 2.5, sy - 1.8);
        } else {
      ...
      if (d.obs && d.obs.trim()) {
        doc.setFont("helvetica", "normal");
        const oL = doc.splitTextToSize(d.obs.trim(), W - 30);
        doc.text(oL, 15, y);
        y += oL.length * 5 + 4;
      } else {
        y += 8;
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10.5);
      doc.text("Governador Edison Lobão/MA, " + fv(d.data_ateste || dtExt(new Date())), W - 15, y, {
        align: "right"
      });
      y += 5.5;
      ...
      y += 30;
      doc.text(fv(ctrl.nome) || "Thiago Soares Lima", W / 2, y, {
        align: "center"
      });
      y += 5.5;
      doc.text(fv(ctrl.cargo) || "Controlador Geral", W / 2, y, {
        align: "center"
      });
      y += 5.5;
      if (ctrl.portaria) doc.text(fv(ctrl.portaria), W / 2, y, {
        align: "center"
      });
      // Rodapé
      doc.setFont("helvetica", "normal");
      ...
      return {
        blob,
        name: "PROCESSO_" + fv(d.processo || "doc") + "_ALDIR_BLANC.pdf"
      };
    }
    // ── FIM Aldir Blanc ───────────────────────────────────────────────────────
...
function NovoProcessoPage({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: iStyle
  }))), form.tipo === "aldir_blanc" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(SH, {
    icon: "\uD83C\uDFE6",
    ...
    value: form.contaBanc,
    onChange: e => upd("contaBanc")(e.target.value),
    placeholder: "00000-0",
    style: iStyle
