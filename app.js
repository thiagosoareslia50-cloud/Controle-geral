...
// ─── Auth ─────────────────────────────────────────────────────────────────────
// [🛡️ Sentinel] Fix: Stronger password hashing (PBKDF2)
// Old un-iterated SHA-256 hash kept for backward compatibility and transparent upgrade
...
async function checkLogin(login, senha) {
  ...
  const currentHash = await hashSenha(u.salt, senha);
  if (currentHash === u.senha) return u;

  // Backward compatibility + opportunistic upgrade
