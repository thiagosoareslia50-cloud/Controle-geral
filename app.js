...
async function importarExcel(file) {
  ...
  // raw:true → XLSX.js NÃO avalia fórmulas, retorna a string "=L2+1" como está
  // Isso evita que fórmulas cumulativas (=L2+1, =L3+1...) gerem valores inflados
  const wb = XLSX.read(buf, {
    type: "array",
    cellDates: true,
    raw: true        // CRÍTICO: não resolver fórmulas
  });
  ...
  return rows.map(row => {
    ...
    for (const [k, v] of Object.entries(row)) {
      const colCanon = canonCol(k);
      let valor = v;

      // Ignorar células que são fórmulas não resolvidas (começam com "=")
      if (typeof valor === "string" && valor.startsWith("=")) {
        valor = "";
      }

      // Para NÚMERO DO DOCUMENTO: garantir que é inteiro válido
      if (colCanon === "NÚMERO DO DOCUMENTO") {
        const n = parseInt(String(valor).trim(), 10);
        valor = !isNaN(n) && n > 0 ? n : "";
      }

      // Para VALOR: limpar e normalizar
      if (colCanon === "VALOR") {
        const s = String(valor).trim();
        if (s && !isNaN(parseFloat(s.replace(",", ".")))) {
          valor = s;
        }
      }

      // Para datas: formatar se for objeto Date
    ...
    return out;
  // Filtrar linhas sem número de documento válido (linhas vazias/fórmulas)
  }).filter(row => {
    const n = parseInt(String(row["NÚMERO DO DOCUMENTO"] || "").trim(), 10);
    return !isNaN(n) && n > 0;
  });
}
...
function App() {
  ...
  useEffect(() => {
    ...
    // Recupera âncora de numeração da última importação
    ST.get("imported_max_num").then(d => {
      if (d && Number.isInteger(d) && d > 0) setImportedMaxNum(d);
    });
  }, []);
  ...
  const onSave = useCallback(async (row, form) => {
    ...
    // Atualizar âncora de numeração se o número salvo for maior que a âncora atual
    const numSalvo = parseInt(String(row["NÚMERO DO DOCUMENTO"] || "").trim(), 10);
    if (!isNaN(numSalvo) && numSalvo > importedMaxNum) {
      setImportedMaxNum(numSalvo);
      ST.set("imported_max_num", numSalvo);
    }
    // Histórico
    const hRow = {
  ...
  const handleImport = useCallback(rows => {
    ...
    // Calcula o maior número da planilha importada (âncora)
    const maxDaPlanilha = maiorNumero(rows);
    // Considera também processos já existentes (edge case: manual > importado)
    const maxExistente = maiorNumero(merged);
    const novaAncora = Math.max(maxDaPlanilha, maxExistente);
    const proximoNum = novaAncora + 1;
    // Persistir âncora no storage para sobreviver a recarregamentos
    if (novaAncora > 0) {
  ...
  // Parte do maior entre: âncora da última importação e maior número atual dos processos
  const nextProcessoNumber = useMemo(() => {
    const baseProcessos = proxNumero(processos);           // maior atual + 1
    const baseAncora = importedMaxNum > 0 ? importedMaxNum + 1 : 1; // âncora + 1
    const candidato = Math.max(baseProcessos, baseAncora);
    // Verificar se o candidato está livre (anti-duplicata final)
