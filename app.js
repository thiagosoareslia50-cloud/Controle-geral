...
async function importarExcel(file) {
  ...
  for (let r = 1; r <= range.e.r; r++) {
    ...
    // Só incluir linhas que tenham NÚMERO DO DOCUMENTO válido
    const nd = row["NÚMERO DO DOCUMENTO"];
    if (temDado && nd !== "" && nd !== undefined) rows.push(row);
  }
...
function App() {
  ...
  const handleImport = useCallback((rows, lastNum) => {
    ...
    const maxManual = maiorNumero(enriched);
    const novaAncora = Math.max(lastNum || 0, maxManual);
