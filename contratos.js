...
window.ContratosPage = function ContratosPage({ historico, processos, dark, contratosMeta, setContratosMeta, toast, user, orgaosConfig, salvarOrgaos }) {
  ...
  const [editForm, setEditForm] = React.useState({ valorTotal: 0, validade: "", aditivos: 0 });
  ...
  const contratosList = React.useMemo(() => {
    ...
    return Object.values(map).map(c => {
      ...
      return {
        ...c, fornecedor, orgao, modalidade, objeto, valorExecutado, meta, valorTotal, aditivos, totalComAditivos, saldo, progresso,
