...
function gerarPDFParecer(d) {
  ...
  const cNome = ctrl.nome || "Thiago Soares Lima";
  const cCarg = ctrl.cargo || "Contador de Controle Interno";
  const cPort = ctrl.portaria || "Portaria 030/2026";
  const hoje  = new Date();
...
function ParecerPage({ processos, dark, toast, appConfig }) {
  ...
  async function handleGerar() {
    ...
    try {
      if (!window.jspdf) await window.loadJsPDF();
