...
function toISO(raw) {
  ...
  const m = s.match(/(\d{1,2})\s+de\s+([a-zA-ZçÇ]+)\s+de\s+(\d{4})/i);
  if (m) {
    const day = m[1].padStart(2, "0");
    const monthName = m[2].toLowerCase();
    const year = m[3];
    const months = {
      janeiro: "01", fev: "02", fevereiro: "02", marco: "03", março: "03", abril: "04",
...
function validarSIAFEM(processos) {
  ...
  processos.forEach((p, i) => {
    ...
    if (cnpj && cnpjRaw.length !== 14 && cnpjRaw.length !== 11) {
      erros.push(`Processo ${num}: CNPJ/CPF "${cnpj}" tem ${cnpjRaw.length} dígitos (esperado 11 ou 14)`);
    }
    ...
    // CNPJ/CPF: dígitos verificadores válidos
    if (cnpjRaw.length === 14 && !validarCNPJ(cnpjRaw)) {
      erros.push(`Processo ${num}: CNPJ "${cnpj}" falhou validação de dígitos`);
    } else if (cnpjRaw.length === 11 && !validarCPF(cnpjRaw)) {
      erros.push(`Processo ${num}: CPF "${cnpj}" falhou validação de dígitos`);
    }
  });
  return erros;
}
...
function cleanContrato(c) {
  if (!c) return "";
  const s = String(c).trim();
  const match = s.match(/(\d+[\d./]*\s*\/\s*\d{4})/);
  if (match) {
    return match[1].replace(/\s+/g, "");
  }
  return s.replace(/^(CONTRATO|CONTRATO ADITIVO|CONTRATO N|CONTRATO Nº|Nº CONTRATO|N CONTRATO|CONTRATO N|Nº|N|NUMERO)\.?\s*/i, "").trim();
}
